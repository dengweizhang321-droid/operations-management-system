...
接收器随系统启动已有候选实现，尚未生产采用：受控发布并启用后，Windows 登录启动或手动启动系统会在数据库、AI 服务和 Worker 就绪后自动开启钉钉接收器。单独手动停止接收器会同时关闭其自动启动；正常停止整套系统保留配置。配置方式与当前限制见 [AI 定时任务的接收器启动说明](docs/AI_DINGTALK_SCHEDULES.md#接收器随系统自动启动候选尚未生产采用)。
