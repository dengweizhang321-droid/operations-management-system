...
2026-09-15，广东仓手动健康规则已按要求修订，待受控发布：库存减少或不变时保留人工健康，只有实物库存较上一当前快照增加时恢复系统判定；先降后升也生效。型号明细列同步改为“备货数量 / 下单剩余库存”，剩余量按备货数量减去下单后每日实物库存正向增量累计，缺数显示待核算。详见 [广东入仓监控规则](docs/INVENTORY_MANAGEMENT.md)。
