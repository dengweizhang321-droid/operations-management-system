...
第二十二批候选代码为较大的固定预算增加独立不可变参数与报告引用，避免把最多 48000 字节参数塞入 32768 字节报告快照。参数有独立用户/全局额度，读取重新核验封存并确定性重算；旧预算报告保留。该底座仍未开放新预算 Agent，需继续接执行协议、完整读取证明与实际模型准入后才能从工作台使用。新迁移仅在隔离环境验证，未用于正式数据库。
