...
- 亿用已批准作为下一个 P/M 直连、每日 M 试点；目标注册表间隔为 1，候选模板保持原 ID、14:20 和 `active=false`。这不代表已部署或通过真实执行：必须先处理旧 M 活动槽、受控迁移持久节奏、采用配套 helper，再替换同一 n8n ID 并完整验收。其他四店节奏不变，具体门禁见 [亿用试点](docs/天猫亿用直连每日M试点.md)。
