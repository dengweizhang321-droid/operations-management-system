...
容量扩展候选底座新增独立来源目录和逐卷 HTML/Excel 渲染，按完整来源、连续行片段与文件摘要核验交付。第二十一批接通首次 v2 工作台；19 来源合成任务从预览、真实领域读取器采集、五 Agent 流程及复核接口到 156 张表、两卷文件已贯通，模型文案使用夹具。超限仍完整展示并拒绝创建，旧任务及备份版本保持兼容。v2 固定预算入口继续开发，合成验收不代表真实数据或模型决策效果通过。详见 [容量实现与后续依赖](docs/AI_BUSINESS_ANALYSIS_CAPACITY.md)。

第二十二、二十三批候选代码为固定预算增加独立不可变参数与报告引用，并接通专用多 Agent 协议、逐 Agent 完整读取证明、调用前容量预检和多卷预算交付。原参数修改须创建新版本，旧报告保留；最多 100 个目标不保证模型能够容纳，超限在模型调用前拒绝。报告 API 已支持固定预算，首次工作台目标选择与试算入口继续接入。新迁移仅在隔离环境验证，未用于正式数据库。
