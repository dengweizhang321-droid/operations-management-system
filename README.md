...
市场图片 AI 识别优先使用系统已验证的 R2 图片缓存，缺失时才回源京东图床；交互式批量识别采用双通道、每通道每次只处理一张图片，同一任务服务端最多保留两个有效推理租约。视觉模型单次调用最多等待 90 秒，浏览器请求最多等待 110 秒，推理租约为 3 分钟；供应商限流、模型超时或网络异常时会安全暂停，再次点击将续跑同类目、同 Prompt、同模型的原任务，已完成结果不会丢失或重复识别。
