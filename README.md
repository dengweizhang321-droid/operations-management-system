...
2026-09-15 **竞态修复已准备，待受控生产采用**：完整停机/重启由唯一 Worker 引擎连续持锁协调，多步部署使用持久维护标记；上新销售周报隔离旧响应，市场 Prompt 按类目原子分配版本。无数据库迁移。首次上线须同时采用 Worker/Django 两侧并协调旧启动入口，不能把源码修复当成生产已经生效。操作与验证见 [竞态修复说明](docs/RACE_CONDITION_FIXES.md)。
