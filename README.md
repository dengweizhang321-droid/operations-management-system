...
市场分析的 SKU 主图标准售价经 AI 识别、人工复核并正式入库后，后续月份若三级类目、榜单口径、SKUID 和图片均一致，系统会直接沿用历史标准价，不再重复 AI 标注或批量入库；图片变化时仍需重新标注。商品榜单识别到正式市场定位价格（主图价）后，该行成交均价按同一主图价展示；未识别到主图价时才按成交数据计算。
