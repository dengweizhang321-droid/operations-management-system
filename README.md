...
2026-09-12 **绿色主题与跨模块沙箱修复候选**：公共界面参考 AI 对话统一配色，移除沙箱配置页面但保留计算能力，修复分页数据集导出与中文单位结果列。已在隔离 PostgreSQL 和真实容器验证销售、ERP、库存关联；本机 3100 为合成数据预览，尚未合并或生产采用。范围与限制见 [候选说明](docs/UNIFIED_THEME_CROSS_APP_SANDBOX.md)。
