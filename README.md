...
- 当前本机 n8n 的吉客云日调度保持停用。2026-09-06 DPAPI 登录、库存查询识别及导出前失败恢复已受控发布；841/842 已保留原证据闭合。最新完整 execution `843` 通过登录、244 个仓库选择及 25,595 行查询验证，在“导出所有页”菜单定位处停止，没有下载或导入。843 已记录 exportIntent，必须保留原运行，不能套用登录/查询失败恢复或直接重跑。当前 release 与验收证据见五表文档；不能把隔离测试通过称为五表链路完成。
