...
- 数据导入页还支持年度利润表中的 SKU 快递费率：工作簿必须包含名称精确为 `SKU累计` 的子表，并以 B 列“代码”、M 列“实际金额”、Z 列“合计快递费”和 AA 列“快递费占比”为固定契约。服务端按 `合计快递费 / 实际金额` 重新计算（实际金额为 0 时按 0），同时核验 AA 缓存值；完整权威集合按规格代码原子替换并回查，冲突重复行会失败关闭。负值或超过 100% 的源数据不静默截断，只记告警并在商品经营明细的“退货率”后展示；未匹配规格显示 `—`。超过 2 MiB 的文件通过 2 MiB 分片上传，最大 128 MiB。
...
- 2026-09-01 已完成网店分析 Django/PostgreSQL 后端候选实现、全量系统测试，以及 1,064,692 条事实的隔离迁移和 D1 终态退役演练；网店分析继续使用现有 React/Next.js 前端，公开 Worker 仍是鉴权、签名、解析与有界转发层。该结果只证明候选链路可切换；本机正式 D1、正式 PostgreSQL、Worker effective head 和 `127.0.0.1:8021/8022` 尚未切换或启用，生产网店事实仍由既有 Worker/D1 路径负责。正式切换必须按完整垂直门禁一次性完成，不能只迁 PostgreSQL；架构、证据和受控步骤见 [`docs/DJANGO_NETSHOP_MIGRATION.md`](docs/DJANGO_NETSHOP_MIGRATION.md)。
...
2026-09-01，网店域 Django/PostgreSQL 后端候选代码与隔离迁移/退役演练已经通过，但生产尚未 cutover。候选拓扑保留现有 React/Next.js 网店前端，新增独立 `netshop_reader`/`netshop_writer`（预留 `127.0.0.1:8021/8022`）、PostgreSQL 单写事实和 Django 有界 consumer；公开 Worker 只保留鉴权/签名/解析/转发边界，市场只保留从 Django 原子刷新的窄兼容投影。由于候选 Worker 不提供 D1 fallback，正式发布必须把数据、authority、服务、Worker API、自动化和所有跨模块消费者作为一个受控垂直切换单元。用户已授权在全部门禁通过后执行正式切换；完整手册见 [`docs/DJANGO_NETSHOP_MIGRATION.md`](docs/DJANGO_NETSHOP_MIGRATION.md)。
