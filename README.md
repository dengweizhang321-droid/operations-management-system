...
- 天猫 n8n 流程固定按 A→B→C→P→M 串行：A/B/C/P 每日完成商品日与推广下载、导入和回查。亿玖 M 每日执行，成功后把下一到期日推进到次日；其余五店继续按独立三日节奏错峰。失败不推进，并在下一次新的完整流程补跑。未到期返回 `not_due`，不创建货品任务，但仍关闭该店 Chromium、释放 helper；n8n 手动完整运行明确强制 M。M 到期失败不会回滚已完成回查的商品日或推广数据，但整个 execution 仍失败并保留原货品活动清单，重跑不得重置已发生的业务点击。日常 Chromium 和其他店铺实例不会被扫描或关闭。
...
- 2026-09-04，工作计划、协作、模板、附件元数据/清理队列及巡店/评价记录的 Django/PostgreSQL 全链路重构和当前生产形状的隔离迁移演练已完成；Worker 只保留鉴权、scope、R2 附件字节和薄适配，AI、全局搜索与库存执行事项改用有界 Django consumer。该范围尚未执行生产 authority 切换：生产 D1 仍是这些事实的当前权威，R2 附件字节未迁移或删除。正式切换必须在单一受控窗口完成冻结、精确迁移、双 authority 激活、真实 release 系统冒烟和 D1 终态退役，不能把代码合并或隔离演练称为生产迁移完成。完整门禁见 [`docs/DJANGO_WORKFLOW_MIGRATION.md`](docs/DJANGO_WORKFLOW_MIGRATION.md)。
- 运营事务新增“上新跟进”：由用户维护产品线展示名称，以吉客云货品代码为唯一归集键，按运行机器本地时区查看完整自然周销售、环比、退款、毛利与上新累计。吉客云货品主数据导入后会触发代码学习，唯一命中自动补入，多义命中不自动归类。机器人配置集中在“系统设置 → 钉钉机器人”，上新跟进图片预览区提供直达入口；钉钉周报发送器在执行时动态唯一核验“志高助手”与“测试群聊”，并使用 PostgreSQL 投递账本阻止同一报告周重复发送。默认未激活的 n8n 调度模板为 `automation/n8n/new-product-weekly-dingtalk.workflow.json`，页面配置和生产调度默认关闭，启用须走受控发布流程。详见 [`docs/NEW_PRODUCT_WEEKLY_FOLLOWUP.md`](docs/NEW_PRODUCT_WEEKLY_FOLLOWUP.md)。
...
2026-09-03，本机运营事务新品项目子域已完成 Django/PostgreSQL 正式单写 cutover、D1 终态退役和新品 R2 路径下线。独立 `workflow_reader`/`workflow_writer` 固定监听 `127.0.0.1:8061/8062` 并加入受控启动链；PostgreSQL 是结构化新品项目、多店目标、七阶段、活动、revision、request receipt 和全部新品读写的唯一权威，生产模式固定为 `django`。旧 D1 `launch` 事实已清除，authority 已变为空 tombstone view，3 个永久 guard 拒绝旧写入复活；新品 R2 候选命名空间为空且生产代码不再可达。工作计划、巡店、评价、变量配置及其附件当前生产仍使用各自 D1/R2 权威；其 Django 全板块实现与隔离迁移演练已于 2026-09-04 完成，但在正式 operations authority 切换前不得视为生产迁移完成。全局 binding 未删除，完整证据与恢复边界见 [`docs/DJANGO_WORKFLOW_MIGRATION.md`](docs/DJANGO_WORKFLOW_MIGRATION.md)。
...
库存总览固定使用销售最新截止日向前 30 个自然日的正向销量，退款不冲减备货需求；备货计划中的“近30天总销量”按货品跨仓汇总，部门预填“志高项目组”但允许留空。分仓库存导入读取吉客云“规格默认供应商”，并使用 `config/inventory-warehouse-mapping.json` 中的仓库分类和“计入库存”标记；“销量近30天”工作台单独展示供应商（代发仓）、京东仓和菜鸟仓等指标。详细口径见 [`docs/INVENTORY_MANAGEMENT.md`](docs/INVENTORY_MANAGEMENT.md)。
