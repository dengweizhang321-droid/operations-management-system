...
**2026-09-10 已上线 AI 回复异常修复**：修正销售工具日期/响应契约、聊天工具轮次间推理字段丢失，并为临近时限及确认完成却无正文的 GLM 响应增加有界收尾。未扩大模型额度、修改模型配置或自动重发网络未知请求。销售和库存真实对话均完成回复、保存与刷新恢复；故障原因、验证边界和正式版本见 [对话异常修复记录](docs/AI_MODULE_CONVERSATIONS.md) 与 [生产证据](docs/evidence/ai-chat-completion-production-20260910.json)。
