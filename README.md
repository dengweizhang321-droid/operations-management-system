...
- 天猫丽力、拓丰、亿用、炊之王、马思图五店已经配置独立 `userDataDir/profileDir/debugPort/downloadDir` 和 `windows_dpapi_credentials` 登录模式，但默认保持 `enabled=false`。仓库提供对应的五个 `active=false` n8n 模板，建议在 11:05、11:10、11:15、11:20、11:25 错峰触发；它们与亿玖店共同使用同一原子 helper 门禁，所以实际业务阶段只能串行。运行 `npm run tmall:n8n:generate` 可从亿玖基线重新生成六店模板，不会发布或激活工作流。
