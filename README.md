...
系统数据集 API 已提供源码实现：243 个数据集（219 个权威记录数据集 + 24 个分析数据集），覆盖销售、ERP、财务、网店、市场、商品、库存、运营事务、客服、权限、AI 和 BI 全部 12 个领域，支持目录、字段 schema、筛选和连续分页，AI 可通过中央工具或既有 MCP 调用。记录数据集仅向无数据范围限制的管理员开放，私有 AI 内容按本人过滤，凭据、原始客户聊天和文件字节不进入通用查询。无需复制业务表或新增迁移；正式采用需要配套 Worker、Django 和各域 reader 列级授权，当前未发布。见 [API 调用说明](docs/SYSTEM_DATASETS_API.md) 与 [完整覆盖清单](docs/SYSTEM_DATASETS_COVERAGE.md)。
