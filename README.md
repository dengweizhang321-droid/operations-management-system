...
多 Agent 深度经营分析按“数据基础 → 协同闭环 → 跨域诊断 → 工程级文件 → 工作台验收”推进，涵盖店铺、品类、SPU、SKU、关键词、同比环比、市场与 B 端。候选分支已支持固定证据预算情景、在线参数试算、新版本重新分析，以及持久 HTML/XLSX 生成和校验下载；下载文件内预算重算、完整自然语言入口和真实业务验收仍待完成。开发进度、数据口径和未完成项见 [实施说明](docs/AI_BUSINESS_ANALYSIS.md)；开发代码不代表已经生产采用。
