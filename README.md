...
钉钉 AI 群对话增强已完成候选开发，**尚未合并或发布到本机正式环境**：系统设置中的钉钉机器人新增独立 AI 对话群设置；群内 @ 在原群回复，私聊在原私聊回复；按现有账号权限接入全部系统领域的只读查询。需采用 AI 0008、更新 reader/writer 权限与 Worker，并受控启动接收器后生效。配置步骤、验证和发布门禁见 [钉钉 AI 对话说明](docs/DINGTALK_READONLY_ASK.md)。以下首版记录仍描述当前已发布行为。
