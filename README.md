...
- 五表任务若只在首次库存登录检查失败，可按五表文档使用受控 `jackyun-preflight-recovery.ts` 闭合无业务效果的旧执行，再从 n8n 手动入口启动完整新执行。恢复会核验原 n8n 错误和四类文件路径，保留原计划及 active；有任何导出、下载或导入证据时拒绝该恢复方式。
...
- 当前本机 n8n 的吉客云日调度保持停用。2026-09-06 经用户确认，五表新版 helper 已受控发布至 `20260906T085035Z-568a59bc151c39d7`，仅试跑一次（execution `841`）；首个分仓库存节点在登录识别阶段返回 `login_unknown`，本轮没有导出文件或业务导入。原运行与 active 清单保留，尚未提供跨 execution 自动恢复；需先核查专用登录/加载状态和原运行证据，不能新建重复任务或声称链路已跑通。详见上述五表文档。
