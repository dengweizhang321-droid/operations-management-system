...
源码中的数据导入工作区包含“文件导入、运行记录、链路规则”，已移除数据连续性入口。运行记录展示各业务接口最近的导入批次，链路规则按店铺矩阵展示仓库配置并链接原 n8n 日志；不据此推断工作流完整成功或实时启停状态。页面改版先在独立 worktree 预览，生产采用需另行受控发布。数据范围与维护方法见 [运行记录和链路规则说明](docs/IMPORT_MONITOR.md)。
