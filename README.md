...
容量扩展候选底座新增独立来源目录和逐卷 HTML/Excel 渲染，按完整来源、连续行片段与文件摘要核验交付。目录已接通显式 API 采集与分析，多卷模块尚未接入持久任务和下载链路；首次工作台仍执行原有 12 来源等限制，旧任务及备份版本保持兼容。详见 [容量实现与后续依赖](docs/AI_BUSINESS_ANALYSIS_CAPACITY.md)。
