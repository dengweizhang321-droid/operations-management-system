...
2026-09-15 **定时截图支持可编辑固定文案**：在 AI 助理 → AI定时任务 → 编辑中填写“附带文案”，每次准备好截图后，先由机器人发送原文，再发完整图片；留空只发图。原新品周报任务已设置标题“新品周销量趋势数据”，继续每天 09:00 发给原目标。截图真实接收已获用户确认，新增组合投递待下一次定时执行验证。见 [发布核验](docs/evidence/scheduled-caption-production-20260915.json)。
