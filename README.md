...
- 京准通 AI 推广明细提供两条默认未激活的 n8n 工作流。`automation/n8n/jd-promotion-daily.workflow.json` 固定绑定 `jd-yiyong-director`（志高商用设备旗舰店、Default profile），手动入口当前补跑 2026-08-20，定时入口每天上海时间 10:30 处理昨天；`automation/n8n/jd-promotion-cut-meat-20260813-14.workflow.json` 固定绑定 `jd-maidehao-operator1`（志高切肉机旗舰店、Profile 2、shopId 745866），只提供手动入口并固定筛选 2026-08-13 至 2026-08-14。设备旗舰店的定时和手动入口都先通过原子协调门禁领取共享 helper，未获授权时每 5 分钟等待、累计 72 次后失败关闭；领取成功后由 A 显式固化 execution ID、店铺和日期，B 从“自定义报表 → AI推广数据自动下载”生成或接管唯一任务、校验 UTF-8 CSV 后按 `jd_promotion/ad` 精确导入并回查，C 独立重读文件和 completed 批次。两条流程与其他京东流程共用全局 Chromium 锁，不得并发。命令行可使用 `npm run jd:promotion -- --store-key <受控店铺键> --start-date YYYY-MM-DD --end-date YYYY-MM-DD`。设备旗舰店说明见 [`docs/京准通AI推广数据n8n工作流.md`](docs/京准通AI推广数据n8n工作流.md)，切肉机店说明见 [`docs/京准通志高切肉机AI推广数据n8n工作流.md`](docs/京准通志高切肉机AI推广数据n8n工作流.md)。
