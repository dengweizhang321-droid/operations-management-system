...
2026-09-17 **经营分析 v2 采集接入为候选代码**：显式目录任务支持最多 48 来源的分页采集、暂停恢复、封存、进度和来源目录读取；事实仍限 64 MiB/2000 页，目录及检查点计入共享额度。工作台可查看和控制 v2，首次创建仍走旧版，v2 报告与持久多卷文件尚未接入。独立多 Agent 开发/复审及隔离 PostgreSQL 425 项（423 通过、2 跳过）完成；未合入 main、生产迁移或部署。详见 [阶段进度](docs/AI_BUSINESS_ANALYSIS.md) 和 [候选证据](docs/evidence/ai-business-v2-collection-candidate.json)。
...
容量扩展候选底座新增独立来源目录和逐卷 HTML/Excel 渲染，按完整来源、连续行片段与文件摘要核验交付。目录与多卷模块尚未接入公开任务和下载链路，当前工作台仍执行原有 12 来源等限制；旧任务及备份版本保持兼容。详见 [容量实现与后续依赖](docs/AI_BUSINESS_ANALYSIS_CAPACITY.md)。
