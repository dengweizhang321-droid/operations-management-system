...
多 Agent 深度经营分析按“数据基础 → 协同闭环 → 跨域诊断 → 工程级文件 → 工作台验收”推进，涵盖店铺、品类、SPU、SKU、关键词、同比环比、市场与 B 端。候选分支已支持固定证据预算情景、在线参数试算、新版本重新分析，以及持久 HTML/XLSX 生成和校验下载；HTML 可离线调整预算并下载未复核试算，Excel 新版附独立参数与公式联动页，超出精确计算范围时明确留空，原报告保留。完整来源容量、自然语言入口和真实业务验收仍待完成。开发进度、数据口径和未完成项见 [实施说明](docs/AI_BUSINESS_ANALYSIS.md)；开发代码不代表已经生产采用。

候选报告页已接入后台 HTML/Excel 双文件生成，支持查看进度、暂停、恢复、取消和下载完整性校验。文件复用同一封存证据及诊断，生成无需保持页面打开；正式文件须先人工复核。该能力尚未部署，完整自然语言入口、预算情景、来源容量扩展和真实业务验收仍在开发。
