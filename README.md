...
- 当前本机 n8n 的吉客云日调度保持停用。2026-09-06 DPAPI 登录与首个登录失败恢复已受控发布；841 已保留原证据闭合。下一次完整 execution `842` 通过登录及 244 个仓库选择，在查询刷新证明处停止，没有导出或导入。已观察到实际 `stockSkuList` 查询 HTTP 200，后续修复及验收状态见五表文档；不能把登录或隔离测试通过称为五表链路完成。
