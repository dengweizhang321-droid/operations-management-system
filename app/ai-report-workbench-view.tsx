...
async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (detailPath && !allowReportDetailBytes(data, responseBytes, response.ok)) throw new Error("报告详情超过此协议的完整响应上限，不能裁剪显示。");
...
export default function AiReportWorkbenchView({ kind }: { kind: Kind }) {
  ...
  return <div className="ai-report-workbench">
    ...
    {detail && <section className="panel report-detail"><div className="section-header"><div><h3>{detail.item.name}</h3><small>{detail.item.id} · {statuses[detail.item.status]}</small></div><div className="report-actions"><button onClick={() => void openDetail(detail.item.id)}>刷新详情</button><button onClick={() => { detailGeneration.current++; detailController.current?.abort(); setDetail(null); }}>关闭</button></div></div><p>{detail.item.scope.platform} · {detail.item.scope.shop} · {detail.item.scope.startDate} 至 {detail.item.scope.endDate}</p><p>模板：{detail.item.template} · 资源库 v{detail.item.version} · 方法：{detail.item.skills.join("、")}</p><div className="report-flow">{detail.workflow.nodes.map(n => <span key={n.id}>{n.key} · {statuses[n.status] || n.status}</span>)}</div>{detail.contentError && <p className="report-note">{detail.contentError}</p>}{detail.sections?.map(s => <section key={s.title}><h4>{s.title}</h4><p className="report-body">{s.body}</p></section>)}{!isScreening && <details><summary>原始节点输出与复核依据</summary>{detail.workflow.nodes.map(n => <div key={n.id}><h4>{n.key}</h4><pre>{JSON.stringify(n.output ?? {}, null, 2)}</pre></div>)}</details>}
      ...
      {!isScreening && detail.budget && detail.snapshot?.question && <AiBusinessBudget key={detail.item.id} reportId={detail.item.id} question={detail.snapshot.question} budget={detail.budget} onCreated={id => void openDetail(id)} />}
      {detail.sections && !detail.item.dryRun && (isBusiness ? <AiBusinessReportFiles key={detail.item.id} reportId={detail.item.id} allowFormal={detail.item.status === "completed"} volumeMode={detail.snapshot?.evidenceProtocol === "reference-v2"} formalOnly={detail.snapshot?.executionProfile === "business-agent-screening-promotion-reference-v1"} /> : <div className="report-actions"><a className="secondary-button" href={link("html", true)}>下载草稿 HTML</a><a className="secondary-button" href={link("xlsx", true)}>下载草稿与取数明细</a></div>)}
