...
export default function GuangdongInventoryView({ canManage, filters, onFiltersChange, onAskAi }: {
  ...
  return <div className={styles.root}>
    <section className="inventory-sync-bar"><div><h2>广东入仓库存监控</h2><p>库存快照 {data?.sync.inventoryAsOf ?? "—"} · 销售截至 {data?.sync.salesThrough ?? "—"} · 固定仓库：广东仓</p></div><div className={styles.actions}>
    ...
    {panel && <section className={`panel ${styles.management}`} aria-label={panel === "watchlist" ? "监控清单管理" : "供应商周期管理"}>
      ...
      </> : <><p>填写从下单到可用入库的完整天数。同一供应商共用设置，安全缓冲默认7天。</p>{cycles?.items.length === 0 && <p>清单暂无已映射供应商，请先添加型号并补齐ERP供应商。</p>}<div className="data-table-wrap"><table className="data-table"><thead><tr><th>供应商</th><th>周期 / 缓冲（天）</th></tr></thead><tbody>{cycles?.items.map((item) => <tr key={`${item.supplier}:${cycles.version}`}><td>{item.supplier}</td><td><form className={styles.actions} onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void run(async () => { await jsonRequest("/suppliers", jsonBody({ action: "supplier", supplier: item.supplier, leadDays: Number(form.get("lead")), bufferDays: Number(form.get("buffer")), version: cycles.version }, "PATCH")); setNotice("供应商周期已保存并回查。"); await loadManagement("suppliers"); setRefresh((value) => value + 1); }); }}><input name="lead" aria-label={`${item.supplier}备货周期`} type="number" min={1} max={365} step={1} required defaultValue={item.leadDays ?? ""} placeholder="待设置" disabled={!canManage || busy} /><input name="buffer" aria-label={`${item.supplier}缓冲天数`} type="number" min={0} max={365} step={1} required defaultValue={item.bufferDays} disabled={!canManage || busy} />{canManage && <button className="primary-button" disabled={busy}>保存</button>}</form></td></tr>)}</tbody></table></div></>}
    </section>}
    {data && <>
      {data.watchCount === 0 ? <section className="panel data-state"><h3>还没有启用的监控型号</h3><p>添加吉客云货品编码后，将自动关联广东仓库存、出库和供应商。</p><button className="primary-button" onClick={() => openPanel("watchlist")}>{canManage ? "添加或导入监控型号" : "查看监控清单"}</button></section> : <>
        ...
        <section className={`panel ${styles.healthPanel}`}><div className={styles.heading}><div><h3>风险库存健康分布</h3><p>按搜索、品牌、品类及供应商统计；点击分类查看明细。</p></div><button className="row-action" onClick={() => changeRisk("")}>全部风险</button></div>
          <div className={styles.distribution}>{data.distribution.map((item) => <button key={item.risk} aria-pressed={risk === item.risk} className={`${styles.riskCard} ${styles[item.risk]}`} onClick={() => changeRisk(risk === item.risk ? "" : item.risk)}><strong>{item.label}</strong><b>{item.itemCount} <small>个型号 · {formatRate(item.itemRate)}</small></b><progress max={1} value={item.quantityRate} /><span>{formatCount(item.quantity)} 件 · {formatRate(item.quantityRate)}</span><span>{formatCurrencyFromCents(item.knownStockValueCents)} · {formatRate(item.valueRate)}</span></button>)}</div>
          <p className={styles.note}>周期包含下单至可用入库，缓冲默认7天。缺成本货值不按零元解释；当前缺库存记录 {data.metrics.missingStockCount} 个型号。</p>
        </section>
        <section className="panel table-panel"><div className="table-toolbar"><h3>广东入仓型号明细</h3><span>{data.pagination.total} 条 · 第 {page} / {Math.max(1, data.pagination.totalPages)} 页</span></div><div className="data-table-wrap" aria-busy={loading}><table className="data-table"><thead><tr>{["货品 / 规格", "供应商", "库存 / 在途", "7 / 30 / 90日出库", "周转 / 库龄", "周期 / 缓冲", "最晚下单", "成本 / 货值", "风险及原因"].map((title) => <th key={title}>{title}</th>)}</tr></thead><tbody>{data.items.map((item) => <tr key={item.productCode}>
          <td><strong>{item.productName}</strong><small className="cell-note">{item.productCode} · {item.specification || "无规格"}</small></td>
          ...
          <td>{numberText(item.outbound7dQuantity)} / {numberText(item.outbound30dQuantity)} / {numberText(item.outbound90dQuantity)}</td>
          <td>{daysText(item.turnoverDays)}<small className="cell-note">库龄 {numberText(item.inventoryAgeDays)} 天</small></td>
          <td>{item.leadDays === null ? "待设置" : `${item.leadDays} 天`}<small className="cell-note">缓冲 {item.bufferDays} 天</small></td>
          <td>{item.latestOrderDate ?? "—"}</td>
          <td>{item.unitCostCents === null ? "成本缺失" : formatCurrencyFromCents(item.unitCostCents)}<small className="cell-note">已覆盖 {formatCurrencyFromCents(item.knownStockValueCents)}</small></td>
          <td className={styles.reason}><strong>{item.riskLabel}</strong><small className="cell-note">{item.riskReasons.join("；") || "可售天数充足"}</small></td>
        </tr>)}{data.items.length === 0 && <tr><td colSpan={9}>当前筛选没有监控结果。</td></tr>}</tbody></table></div><footer className="jd-sku-pagination"><button className="row-action" disabled={page <= 1 || loading} onClick={() => setPageState({ scope, page: page - 1 })}>上一页</button><button className="row-action" disabled={page >= data.pagination.totalPages || loading} onClick={() => setPageState({ scope, page: page + 1 })}>下一页</button></footer></section>
      </>}
    </>}
