  ...
  return <section className="panel sales-overview-filter-panel sales-shared-filter-panel" aria-label={`${scopeLabel}公共筛选`} aria-busy={updating}>
    <div className="sales-overview-filter-heading">
      ...
      <div className="sales-overview-filter-controls">
        <label><span>平台</span><SearchableMultiSelect values={filters.platforms} onChange={updatePlatforms} ariaLabel="销售分析平台" allLabel="全部平台" searchPlaceholder="搜索平台" options={options.platforms.map((platform) => ({ value: platform, label: platform }))} /></label>
        <label><span>店铺</span><SearchableMultiSelect values={filters.outletKeys} onChange={(outletKeys) => patch({ outletKeys })} ariaLabel="销售分析店铺" allLabel="全部店铺" searchPlaceholder="搜索店铺或平台" options={visibleShops.map((shop) => ({ value: shop.key, label: shop.platform === "未分类" ? shop.name : `${shop.platform} · ${shop.name}`, searchText: `${shop.platform} ${shop.name}` }))} /></label>
        {capabilities.categories !== false && <label><span>品类</span><SearchableMultiSelect values={filters.categories} onChange={(categories) => patch({ categories })} ariaLabel="销售分析品类" allLabel="全部品类" searchPlaceholder="搜索品类" options={options.categories.map((category) => ({ value: category, label: category }))} /></label>}
        {capabilities.channels && <label><span>渠道</span><SearchableMultiSelect values={filters.channels} onChange={(channels) => patch({ channels })} ariaLabel="销售分析渠道" allLabel="全部渠道" searchPlaceholder="搜索渠道" options={(options.channels ?? []).map((channel) => ({ value: channel, label: channel }))} /></label>}
