...
const workflowConfigs: Record<WorkflowKey, WorkflowConfig> = {
  ...
  tmall: {
    ...
    safetyNote: "页面只嵌入本机编辑器，Cookie、账号、密码、Token 和 Session 均不进入运营系统。本地 Worker 自动守护一次性环回服务；A→B→C→P→M 绑定同一 n8n execution，默认直接处理昨天，推广完成后才执行货品主数据并在终态关闭本轮受控 Chromium。M 失败不回滚已完成回查的商品日或推广导入，但整个 execution 仍失败并保留货品活动清单；所有导入接口只在业务范围与规范化后的完整业务内容都一致时返回 duplicate。",
    stageDetails: {
