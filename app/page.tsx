...
import MarketView, { MarketDataImportPanel, MarketWorkflowPanel } from "./market-view";
...
function SettingsView({ currentUser }: { currentUser: CurrentUser | null }) {
  ...
  return <><div className="subnav"><button className={activeTab === "parameters" ? "active" : ""} onClick={() => setActiveTab("parameters")}>系统参数</button><button className={activeTab === "master" ? "active" : ""} onClick={() => setActiveTab("master")}>主数据与映射</button><button className={activeTab === "permissions" ? "active" : ""} onClick={() => setActiveTab("permissions")}>权限管理</button></div>{(error || notice) && <section className={`inventory-feedback ${error ? "inventory-feedback-error" : "inventory-feedback-success"}`} role={error ? "alert" : "status"}><span>{error ? "!" : "✓"}</span><div><strong>{error ? "处理失败" : "保存成功"}</strong><p>{error || notice}</p></div></section>}
    ...
    {activeTab === "master" && <section className="settings-market-master"><article className="panel settings-master-overview"><SectionHeader title="TOP SKU 主数据中心" note="管理市场榜单数据库、价格带、细分类目、导入和 AI 标注工作流" /><div className="settings-master-cards"><div><strong>{marketLoading ? "读取中" : formatCount(marketData?.summary.productCount ?? 0)}</strong><span>TOP SKU/SPU</span></div><div><strong>{formatCount(marketData?.summary.pendingAiCount ?? 0)}</strong><span>待确认 AI 数据</span></div><div><strong>{formatCount(marketData?.imageCache.cached ?? 0)} / {formatCount(marketData?.imageCache.total ?? 0)}</strong><span>图片缓存</span></div><div><strong>{marketData?.dataRange.endDate ?? "暂无"}</strong><span>数据截止日期</span></div></div><p>价格带按照 AI 从主图识别并经人工确认的市场定位价划分；源表价格、AI 建议价、成交均价和人工确认价会保留历史快照。</p></article><MarketDataImportPanel currentUser={currentUser} data={marketData} onImported={() => setMarketReloadKey((key) => key + 1)} /><MarketWorkflowPanel data={marketData} /><MarketAnnotationView currentUser={currentUser} /></section>}
