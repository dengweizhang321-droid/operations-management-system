...
export default function MarketAnnotationView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const savePrompt = (mode: "manual" | "generate") => act("prompt", async () => {
    if (!category) {
      if (mode !== "manual") throw new Error("AI 生成 Prompt 前请选择一个具体三级类目");
      const result = await post({ action: "create_prompts_for_all", promptBody, changeNote });
      await load(jobId);
      setNotice(`已为 ${Number(result?.count ?? data?.categories.length ?? 0)} 个三级类目分别创建 Prompt 草稿`);
      return;
    }
  ...
  return <div className="market-annotation-module">
    ...
    <section className="annotation-two-column"><article className="panel annotation-prompt-card"><div className="section-header"><div><h3>3. Prompt 版本与自动进化</h3><p>正文与枚举始终明文可见；编辑只创建不可变子版本。未使用的草稿可以删除，激活及归档版本永久保留。</p></div></div>{error && <div className="market-feedback error">{error}</div>}
      <div className="annotation-prompt-history">{data.prompts.filter((item) => !category || item.category === category).map((item) => <div key={item.id} className="annotation-prompt-version"><button className={`annotation-prompt-select ${selectedPrompt?.id === item.id ? "active" : ""}`} onClick={() => choosePrompt(item)}><strong>v{item.version} · {item.status}</strong><span>{!category ? `${item.category} · ` : ""}{item.source}</span><small>{item.id}</small></button>{isAdmin && item.status === "draft" && <button className="annotation-prompt-delete" disabled={busy !== ""} title={`删除 v${item.version} 草稿`} onClick={() => void deletePrompt(item)}>删除</button>}</div>)}</div>
      {!category && <p className="soft-text">全部模式会把同一份 Prompt 正文分别保存到每个三级类目；细分品类枚举仍按各类目的系统配置独立生成，版本也可分别复核和激活。</p>}
      <label><span>细分品类枚举（每行一个）</span><textarea value={category ? segmentsText : "全部模式按每个三级类目的系统默认细分品类分别保存，不共用同一套枚举。"} disabled={!category} onChange={(event) => setSegmentsText(event.target.value)} /></label><label><span>Prompt 正文</span><textarea className="annotation-prompt-body" value={promptBody} onChange={(event) => setPromptBody(event.target.value)} placeholder="写明视觉分类规则、价格识别口径和严格 JSON 输出" /></label><label><span>版本说明</span><input value={changeNote} onChange={(event) => setChangeNote(event.target.value)} /></label>
      ...
      <div className="annotation-actions"><button className="secondary-button" disabled={!canEdit || busy !== ""} onClick={() => void savePrompt("manual")}>{category ? "保存人工子版本" : `保存至全部类目（${data.categories.length}）`}</button><button className="secondary-button" title={!category ? "AI 生成需选择具体三级类目" : ""} disabled={!canEdit || !category || !textModelId || busy !== ""} onClick={() => void savePrompt("generate")}>AI 生成</button><button className="secondary-button" title={!category ? "AI 进化需选择具体三级类目" : ""} disabled={!canEdit || !category || !selectedPrompt || !textModelId || !visionModelId || busy !== ""} onClick={evolve}>AI 进化并测试</button><button className="secondary-button" title={!category ? "冻结测试需选择具体三级类目" : ""} disabled={!canEdit || !category || !selectedPrompt || !visionModelId || busy !== ""} onClick={() => void testPrompt()}>冻结抽样测试</button>{selectedPrompt?.status === "active" ? <button className="primary-button" disabled>当前激活版本</button> : selectedPrompt && <button className="primary-button" disabled={!isAdmin || busy !== ""} onClick={() => void activate(selectedPrompt, selectedPrompt.status === "archived")}>{selectedPrompt.status === "archived" ? "回滚到此版" : "激活此版"}</button>}</div>
    </article>
