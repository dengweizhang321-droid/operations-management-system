...
type CatalogItem = { skuCode: string; productName: string; brand: string; category: string; imageUrl: string; rankingPriceCents: number | null; annotationId?: string; finalSegment?: string; finalImagePriceCents?: number | null; reviewStatus: string };
...
export default function MarketAnnotationView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const activate = (item: Prompt, rollback = false) => act("activate", async () => {
    const reason = window.prompt(rollback ? "请输入回滚原因（至少6字）" : "若门禁未通过，管理员可输入显式确认原因；已通过可留空", rollback ? "回滚到已验证的稳定版本" : "") ?? "";
    await post({ action: rollback ? "rollback_prompt" : "activate_prompt", promptId: item.id, explicitOverride: !rollback && reason.length >= 6, reason });
  ...
  return <div className="market-annotation-module">
    ...
    <section className="annotation-two-column"><article className="panel annotation-prompt-card"><div className="section-header"><div><h3>3. Prompt 版本与自动进化</h3><p>正文与枚举始终明文可见；编辑只创建不可变子版本。</p></div></div>
      <div className="annotation-prompt-history">{data.prompts.filter((item) => !category || item.category === category).map((item) => <button key={item.id} className={selectedPrompt?.id === item.id ? "active" : ""} onClick={() => choosePrompt(item)}><strong>v{item.version} · {item.status}</strong><span>{item.source}</span><small>{item.id}</small></button>)}</div>
      ...
      <div className="annotation-actions"><button className="secondary-button" disabled={!canEdit || busy !== ""} onClick={() => void savePrompt("manual")}>保存人工子版本</button><button className="secondary-button" disabled={!canEdit || !textModelId || busy !== ""} onClick={() => void savePrompt("generate")}>AI 生成</button><button className="secondary-button" disabled={!canEdit || !selectedPrompt || !textModelId || !visionModelId || busy !== ""} onClick={evolve}>AI 进化并测试</button><button className="secondary-button" disabled={!canEdit || !selectedPrompt || !visionModelId || busy !== ""} onClick={() => void testPrompt()}>冻结抽样测试</button>{selectedPrompt && <button className="primary-button" disabled={!isAdmin || busy !== ""} onClick={() => void activate(selectedPrompt, selectedPrompt.status === "archived")}>{selectedPrompt.status === "archived" ? "回滚到此版" : "激活此版"}</button>}</div>
    </article>
