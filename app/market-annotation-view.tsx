...
export default function MarketAnnotationView({ currentUser, embedded = false }: { currentUser: CurrentUser; embedded?: boolean }) {
  ...
  }, [jobId, search, searchPage, itemPage, category, promptId, itemSegment, storageStatus]);
  ...
  useEffect(() => { setItemPage(1); if (!jobId) return; const timer = window.setTimeout(() => void load(jobId, search, searchPage, 1).catch(() => undefined), 0); return () => window.clearTimeout(timer); }, [itemSegment, storageStatus]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const saveReview = (ids = (data?.items ?? []).filter((item) => ["review_pending", "approved", "rejected"].includes(item.status)).map((item) => item.id)) => act("save-review", async () => {
    ...
    const updates = ids.map((id) => ({ id, version: drafts[id]!.version, segment: drafts[id]!.segment, imagePriceCents: centsInput(drafts[id]!.price), selected: drafts[id]!.selected }));
  ...
  const commit = () => act("commit", async () => {
    ...
    await post({ action: "review", jobId, updates: selectedIds.map((id) => ({ id, version: drafts[id]!.version, segment: drafts[id]!.segment, imagePriceCents: centsInput(drafts[id]!.price), selected: true })) });
  ...
  return <div className="market-annotation-module">
    ...
    <section className="panel annotation-review-card">
      <div className="section-header"><div><h3>2. 人工复核与批量入库</h3><p>支持按细分品类和入库状态筛选；列表适合快速复核，大图适合核对主图价格。</p></div><div className="annotation-actions"><button className="secondary-button" disabled={!canEdit || busy !== ""} onClick={() => void saveReview()}>保存复核</button><button className="primary-button" disabled={!isAdmin || !selectedIds.length || busy !== ""} onClick={commit}>批量入库（{selectedIds.length}）</button></div></div>
