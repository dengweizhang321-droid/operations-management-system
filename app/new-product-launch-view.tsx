...
export function validateStageDraft(draft: StageDraft): string {
  if (draft.status === "blocked" && !draft.blocker.trim()) return "阶段标记为受阻时，请填写阻塞原因。";
...
function ProjectEditor({ project, facets, saving, onClose, onSave }: {
  ...
  return <Dialog open onClose={() => !saving && onClose()} dialogId="new-product-project-editor" ariaLabel={project ? "编辑新品项目" : "新建新品项目"} className="workflow-edit-modal launch-project-modal">
    ...
    <span className="eyebrow">NEW PRODUCT PROJECT</span><h2>{project ? "编辑新品项目" : "新建新品项目"}</h2><p className="launch-modal-intro">先建立商品与目标店铺，再在阶段矩阵中分配每个节点的负责人、截止日和交付证据。</p>
    <form className="workflow-edit-form launch-project-form" noValidate onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      ...
      <label className="workflow-edit-content-field"><span>项目备注</span><textarea rows={3} maxLength={4000} value={draft.notes} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} /></label>
      <fieldset className="launch-target-editor workflow-edit-content-field"><legend>目标店铺（至少 1 个）</legend>{draft.targets.map((target, index) => <div key={`${index}-${target.id ?? "new"}`}>
      ...
      </div>)}<button type="button" className="secondary-button" onClick={() => setDraft((current) => ({ ...current, targets: [...current.targets, emptyTarget()] }))}>＋ 添加目标店铺</button></fieldset>
...
function StageEditor({ project, stage, saving, onClose, onSave }: {
  ...
  return <Dialog open onClose={() => !saving && onClose()} dialogId="new-product-stage-editor" ariaLabel={`编辑${stage.label}阶段`} className="workflow-edit-modal launch-stage-modal">
    ...
    <form className="workflow-edit-form" noValidate onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      <label><span>状态</span><select autoFocus value={draft.status} onChange={(event) => { setError(""); setDraft((current) => ({ ...current, status: event.target.value as StageStatus })); }}>{STAGE_STATUS_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      <label><span>负责人</span><input maxLength={120} value={draft.owner} onChange={(event) => setDraft((current) => ({ ...current, owner: event.target.value }))} /></label>
      <label><span>计划截止日</span><input type="date" value={draft.plannedDueDate} onChange={(event) => setDraft((current) => ({ ...current, plannedDueDate: event.target.value }))} /></label>
      <label className="workflow-edit-content-field"><span>阻塞原因{draft.status === "blocked" ? "（必填）" : ""}</span><textarea rows={2} maxLength={500} value={draft.blocker} onChange={(event) => { setError(""); setDraft((current) => ({ ...current, blocker: event.target.value })); }} /></label>
      <label className="workflow-edit-content-field"><span>节点说明 / 交付结果</span><textarea rows={4} maxLength={2000} value={draft.notes} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} /></label>
      <label><span>证据名称</span><input maxLength={160} value={draft.evidenceLabel} placeholder="例如：主图定稿" onChange={(event) => setDraft((current) => ({ ...current, evidenceLabel: event.target.value }))} /></label>
      <label><span>证据链接</span><input type="url" maxLength={1000} value={draft.evidenceUrl} placeholder="https://…" onChange={(event) => { setError(""); setDraft((current) => ({ ...current, evidenceUrl: event.target.value })); }} /></label>
      {error && <p className="workflow-edit-validation" role="alert">{error}</p>}
...
function ProjectDetail({ project, canWrite, saving, onClose, onEdit, onEditStage }: {
  ...
  return <Dialog open onClose={onClose} dialogId="new-product-project-detail" ariaLabel={`${project.productName}项目详情`} className="workflow-edit-modal launch-detail-modal">
    ...
    <section className="launch-detail-targets"><h3>目标店铺</h3><div>{project.targets.map((target) => <article key={`${target.platform}-${target.shopName}`}><strong>{target.platform} · {target.shopName}</strong><span>{target.status === "listed" ? "已上架" : target.status === "ready" ? "可上架" : target.status === "paused" ? "暂停" : "待准备"}</span><small>{target.listingSku || "SKU 待补充"}</small>{target.listingUrl && <a href={target.listingUrl} target="_blank" rel="noreferrer">打开商品链接</a>}</article>)}</div></section>
    ...
    {project.notes && <section className="launch-detail-notes"><h3>项目备注</h3><p>{project.notes}</p></section>}
...
export default function NewProductLaunchView({ canWrite, legacyFallback }: { canWrite: boolean; legacyFallback: ReactNode }) {
  ...
  const saveStage = async (draft: StageDraft) => {
    ...
    try {
      ...
      await requestJson(`/api/workflow/launch-projects/${encodeURIComponent(project.id)}/stages/${stage.stageKey}`, {
        method: "PATCH",
        body: { ...draft, plannedDueDate: draft.plannedDueDate || null, expectedVersion: stage.version },
      });
  ...
  return <div className="launch-workspace">
    ...
    {loading && items.length === 0 ? <LoadingState /> : view === "matrix" ? <section className="panel launch-matrix-panel data-refresh-region" aria-busy={loading}><header className="workflow-list-heading"><div><h3>新品阶段矩阵</h3><p>横向对齐每个项目的七个标准节点；点击阶段即可维护负责人、期限与交付证据。</p></div><span>已加载 {items.length} / {pagination.total}</span></header><div className="data-table-wrap"><table className="data-table launch-matrix-table"><thead><tr><th>商品 / 供应商</th><th>目标店铺</th><th>提出 / 上架</th>{STAGES.map((stage) => <th key={stage.key}>{stage.label}</th>)}<th>整体</th><th>操作</th></tr></thead><tbody>{items.map((project) => <tr key={project.id}><td><div className="launch-product-cell">{project.productImageUrl ? <img src={project.productImageUrl} alt="" /> : <span>新</span>}<div><strong>{project.productName}</strong><small>{project.supplierName || "供应商待补充"}</small><em>{project.erpProductCode || project.skuCode || project.category || "编码待补充"}</em></div></div></td><td><div className="launch-target-chips">{project.targets.map((target) => <span key={`${target.platform}-${target.shopName}`} title={`${target.platform} · ${target.shopName}`}>{target.platform} · {target.shopName}</span>)}</div></td><td><time>{project.proposedDate}</time><small>目标 {project.targetLaunchDate || "待排期"}</small></td>{STAGES.map((definition) => { const stage = project.stages.find((item) => item.stageKey === definition.key); return <td key={definition.key}>{stage ? <button type="button" className="launch-stage-cell" disabled={!canWrite} onClick={() => canWrite && setStageEditor({ project, stage })}><StageBadge stage={stage} compact /><small>{stage.owner || "未指定"}</small><time>{stage.plannedDueDate || "待排期"}</time>{stage.evidenceUrl && <em>有证据</em>}</button> : "—"}</td>; })}<td><div className="launch-progress"><StatusBadge status={project.status} /><i><b style={{ width: `${project.progressPercent}%` }} /></i><small>{project.progressPercent}%{project.overdue ? ` · ${project.overdueStageCount} 项逾期` : ""}</small></div></td><td><div className="workflow-plan-actions"><button type="button" className="row-action" onClick={() => void openDetail(project)}>详情</button><button type="button" className="row-action" disabled={!canWrite || saving} onClick={() => setEditor(project)}>编辑</button><button type="button" className="row-action danger" disabled={!canWrite || saving} onClick={() => void deleteProject(project)}>删除</button></div></td></tr>)}{items.length === 0 && <tr><td colSpan={12}><div className="table-state">暂无符合条件的新品项目。</div></td></tr>}</tbody></table></div></section> : <section className="launch-kanban data-refresh-region" aria-busy={loading}>{kanban.map((column) => <article className={`panel kanban-${column.value}`} key={column.value}><header><span>{column.label}</span><strong>{column.items.length}</strong></header><div>{column.items.map((project) => <button type="button" key={project.id} onClick={() => void openDetail(project)}><header><b className={`priority-${project.priority}`}>{priorityLabel(project.priority)}</b><small>{project.targetLaunchDate || "待排期"}</small></header><strong>{project.productName}</strong><p>{project.supplierName || "供应商待补充"}</p><div className="launch-mini-stages">{project.stages.map((stage) => <i key={stage.stageKey} className={`stage-${stage.status}`} title={`${stage.label}：${stageStatusLabel(stage.status)}`} />)}</div><footer><span>{project.owner || "未指定负责人"}</span><em>{project.progressPercent}%</em></footer>{project.targets.slice(0, 2).map((target) => <small key={`${target.platform}-${target.shopName}`}>{target.platform} · {target.shopName}</small>)}</button>)}{column.items.length === 0 && <p className="launch-kanban-empty">暂无项目</p>}</div></article>)}</section>}
