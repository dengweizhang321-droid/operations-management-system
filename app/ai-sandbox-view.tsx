  ...
  return <section className="ai-sandbox-workspace">
    ...
    <div className="ai-sandbox-grid">
      ...
      <article className="panel ai-admin-card">
        ...
        {result && <div className="ai-sandbox-result"><div className="ai-sandbox-stats"><span><strong>{result.sourceRows}</strong> 源行</span><span><strong>{result.returned}</strong> 返回</span><span><strong>{result.stepsApplied.length}</strong> 步</span></div><div className="table-scroll"><table data-column-filter-scope={result.truncated ? "none" : "full"}><thead><tr>{result.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{result.rows.map((row, index) => <tr key={`${result.runId}-${index}`}>{result.columns.map((column) => <td key={column}>{row[column] === null || row[column] === undefined ? "—" : String(row[column])}</td>)}</tr>)}</tbody></table></div></div>}
      </article>
    </div>
