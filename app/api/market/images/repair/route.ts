...
function integerParam(value: string | null, fallback: number) {
  ...
  if (!Number.isSafeInteger(parsed)) throw new Error("分页参数必须是整数");
  return parsed;
}
...
export async function GET(request: Request) {
  try {
    ...
    if (auth) return auth;
    return Response.json({ error: error instanceof Error ? error.message : "读取缺图商品失败" }, { status: 400 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const auth = authorizationErrorResponse(error);
    if (auth) return auth;
    return Response.json({ error: error instanceof Error ? error.message : "修复商品图片失败" }, { status: 400 });
  }
}
