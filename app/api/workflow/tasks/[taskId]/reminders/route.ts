...
const pathFor = (taskId: string) => `/api/workflow/tasks/${encodedWorkflowResource(taskId)}/reminders`;
...
export async function GET(request: Request, context: Context) {
  try { const principal = await requireAppPrincipal(["viewer", "analyst", "operator", "admin"]); requireUnrestrictedDataScope(principal, "工作事项提醒"); await getWorkflowBackendMode(); const { taskId } = await context.params;
  ...
  } catch (error) { return routeError(error, "读取提醒失败。"); }
}
export async function POST(request: Request, context: Context) {
  try { const principal = await requireAppPrincipal(["operator", "admin"]); requireUnrestrictedDataScope(principal, "工作事项提醒", "修改"); await getWorkflowBackendMode(); const { taskId } = await context.params;
    const payload = requireWorkflowJsonObject(await request.json().catch(() => null), "提醒内容必须是有效的 JSON 对象。");
  ...
  } catch (error) { return routeError(error, "保存提醒失败。"); }
}
export async function DELETE(request: Request, context: Context) {
  ...
  } catch (error) { return routeError(error, "取消提醒失败。"); }
}
