...
import { marketOptionsPrincipalKey, marketOptionsQuery, validateMarketOptionsResponse } from "@/lib/ai/business-market-options";
...
export async function GET(request: Request) {
  try {
    ...
    const principalKey = await marketOptionsPrincipalKey(principal.email);
    if (principalKey !== expected[0])
      return Response.json({ code: "principal_mismatch", error: "账号已变化，请刷新当前账号后重新选择市场来源。" },
    ...
    const result = await requestMarketAnalysisOptions<Record<string, unknown>>(principal, query, { signal: request.signal });
    let page;
    try {
      ...
      page = await validateMarketOptionsResponse({ schemaVersion: "business-market-options-response-v1", principalKey, page: result.data },
        { principalKey, query: marketOptionsQuery(Object.fromEntries(filters)) });
      if (result.revision !== page.revision) throw new Error("Revision header differs");
    } catch {
      throw new PublicApiError(503, "service_unavailable", "市场来源选项响应未通过完整核验，请重新读取。");
    }
    if (request.signal.aborted) throw new PublicApiError(503, "service_unavailable", "来源选项读取已取消。");
    return Response.json({ schemaVersion: "business-market-options-response-v1", principalKey, page },
