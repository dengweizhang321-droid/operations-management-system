...
export default function AiDingTalkSchedulesView() {
  ...
  const run = async (item: Schedule) => {
    const effect = item.contentType === "screenshot" ? "截取指定系统页面并由机器人发送图片" : item.contentType === "report_file" ? "读取已复核报告并由机器人发送 Excel 文件" : "调用付费 AI 模型并由机器人发送文字";
