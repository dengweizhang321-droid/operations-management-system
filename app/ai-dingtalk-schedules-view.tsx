...
type Schedule = { id: string; name: string; prompt: string; cadence: "daily" | "weekly" | "monthly"; hour: number; minute: number; day: number;
  targetType: "group" | "person"; targetId: string; senderId: string; enabled: boolean; version: number; nextRunAt: string | null; lastRunAt: string | null };
type Run = { id: string; scheduleId: string; scheduledAt: string; status: string; errorCode: string };
type Draft = Pick<Schedule, "name" | "prompt" | "cadence" | "hour" | "minute" | "day" | "targetType" | "targetId" | "senderId" | "enabled"> & { id?: string; expectedVersion?: number };
type Group = { id: string; name: string; enabled: boolean };
const empty = (): Draft => ({ name: "", prompt: "", cadence: "daily", hour: 9, minute: 0, day: 1, targetType: "person", targetId: "", senderId: "", enabled: false });
...
export default function AiDingTalkSchedulesView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(schedules.items); setRuns(schedules.runs); setGroups(settings.config.groups);
  ...
  const run = async (item: Schedule) => {
    if (!window.confirm(`确认立即执行“${item.name}”？可能调用付费 AI 模型并向钉钉${item.targetType === "group" ? "群" : "个人"}发送结果。`)) return;
  ...
  const edit = (item: Schedule) => { setDraft({ id: item.id, expectedVersion: item.version, name: item.name, prompt: item.prompt, cadence: item.cadence, hour: item.hour, minute: item.minute, day: item.day, targetType: item.targetType, targetId: item.targetId, senderId: item.senderId, enabled: item.enabled }); setEditing(true); };
  return <section className="panel ai-admin-card ai-ding-schedules">
    <div className="section-header"><div><h3>AI定时任务</h3><p>按上海时间执行只读 AI 指令，并将结果发给已绑定的本人或已批准的钉钉群。创建任务不会直接发送。</p></div><button type="button" className="secondary-button" onClick={() => { setDraft(empty()); setEditing(true); }} disabled={busy}>新增任务</button></div>
    ...
    {editing && <div className="ai-ding-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setEditing(false); }}><div className="ai-ding-modal" role="dialog" aria-modal="true" aria-label={draft.id ? "编辑定时任务" : "新增定时任务"}>
      ...
      <form className="ai-config-form" onSubmit={event => void save(event)}>
        ...
        <label className="ai-form-wide"><span>执行内容（只读 AI 指令）</span><textarea required maxLength={4000} rows={5} value={draft.prompt} onChange={e => setDraft({ ...draft, prompt: e.target.value })} /></label>
        ...
        <label className="ai-form-wide"><span>执行账号 staffId（必须已绑定系统账号）</span><input required maxLength={160} value={draft.senderId} onChange={e => setDraft({ ...draft, senderId: e.target.value, targetId: draft.targetType === "person" ? e.target.value : draft.targetId })} /><small>任务按此人的现行权限查询。个人投递仅发给该账号本人；若账号失效、群被撤销或机器人未安装，执行失败关闭。</small></label>
