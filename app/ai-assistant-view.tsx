...
function AiAssistantView({ currentUser }: { currentUser: CurrentUser | null }) {
  ...
  const loadConfiguration = useCallback(async (signal?: AbortSignal) => {
    ...
    const [modelsResponse, channelsResponse] = await Promise.all([
      fetch("/api/ai/models", { cache: "no-store", signal }),
      fetch("/api/ai/channels", { cache: "no-store", signal }),
    ]);
  ...
  const loadConversations = useCallback(async (options: { page?: number; append?: boolean; signal?: AbortSignal } = {}) => {
    ...
    try {
      const response = await fetch(`/api/ai/conversations?page=${page}&pageSize=30`, { cache: "no-store", signal: controller.signal });
      const payload = await response.json().catch(() => null) as { items?: AiConversationRecord[]; models?: AiAvailableChatModel[]; pagination?: AiConversationPagination; error?: string } | null;
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      await Promise.all([loadConfiguration(controller.signal), loadConversations({ signal: controller.signal })]);
    } catch (reason) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (conversationId && conversationId === activeConversationId) await loadMessages(conversationId);
    } catch (reason) {
