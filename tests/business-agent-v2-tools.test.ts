...
test("all preexisting catalog canonical hashes and entry metadata remain exactly unchanged", async () => {
  ...
  assert.deepEqual(aiToolSurfaces.filter(surface => surface !== "business_agent_v2" && surface !== "business_agent_budget_v1" && surface !== "business_agent_integrated_v1"), baseline.legacySurfaces);
  assert.deepEqual(aiToolSurfaces.slice(0, baseline.legacySurfaces.length), baseline.legacySurfaces);
  assert.deepEqual(aiToolSurfaces.slice(baseline.legacySurfaces.length), ["business_agent_v2", "business_agent_budget_v1", "business_agent_integrated_v1"]);
  const oldEntries = aiToolRegistry.filter(entry => !names.includes(entry.name) && !["get_business_budget_directory_v1", "get_business_budget_analysis_table_v1", "get_business_budget_scenarios_v1", "get_business_integrated_directory_v1", "get_business_integrated_analysis_table_v1", "get_business_integrated_budget_v1"].includes(entry.name));
