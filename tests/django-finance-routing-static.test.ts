...
test("finance cutover keeps the existing UI and public paths while defaulting to legacy", async () => {
  const [page, analysis, targets, imports, service] = await Promise.all([
  ...
  assert.match(page, /\/api\/imports\/finance/);
  assert.doesNotMatch(page, /TERUISI_DJANGO_FINANCE_MODE|finance-normalized-v1/);
