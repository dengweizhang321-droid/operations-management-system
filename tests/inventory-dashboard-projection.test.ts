...
test("BI 首页显式请求库存 dashboard 轻量投影", async () => {
  ...
  assert.match(overview, /\(SELECT substr\(MIN\(ship_time\), 1, 10\)[\s\S]*\) AS start_date/);
  assert.match(overview, /\(SELECT substr\(MAX\(ship_time\), 1, 10\)[\s\S]*\) AS end_date/);
  assert.doesNotMatch(overview, /MIN\(substr\(ship_time, 1, 10\)\)[\s\S]*MAX\(substr\(ship_time, 1, 10\)\)/);
