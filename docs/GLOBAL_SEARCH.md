...
- API 支持 `page`、`limit`、`totalLimit`，以及单分组分页的 `group`。每组通过 `LIMIT + 1` 判断 `hasMore`，不再为搜索结果执行窗口总数扫描；`totalExact=true` 时 `total` 是精确值，否则 `total` 只是当前页能够证明的已知下界。
- 默认多域搜索最多同时执行 3 组查询，并受 2 秒组查询总截止时间约束；到达截止时间后不会启动后续组。响应通过 `deadlineExceeded` 与 `timedOutDomains` 披露本次不完整域，调用方可指定单一 `group` 重试。
