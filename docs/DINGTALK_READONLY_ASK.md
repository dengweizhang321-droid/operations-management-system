...
## 2026-09-10 群对话增强候选（待发布）

用户要求群内 @ 原群回复、系统设置提供 AI 对话群配置，并开放全板块只读查询。本节描述本分支候选行为；下面首版记录是已发布历史，不代表候选已经上线。
...
5. 实际单聊、测试群 @ 各提问，增加财务/市场/运营事务等代表场景；核对原消息会话、AI 工具 surface、结果回复位置和投递账本。真实模型与平台端到端验收尚未执行，隔离夹具不能替代。
6. 发布后备份并独立恢复验证。失败先停用 AI 对话并受控停止接收器，保留表、权限和审计，使用兼容或前向修复；不删除表或恢复 D1。

候选验证见 [群对话候选证据](evidence/dingtalk-group-ai-candidate-20260910.json)。当前模型响应时间和稳定性限制仍见 [AI 故障复盘](AI_CHAT_RELIABILITY_REVIEW_20260910.md)，本次接入范围扩大不等于所有自然语言分析场景已验收。
