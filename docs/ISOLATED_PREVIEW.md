...
市场分析预览包含 40 条合成商品，支持两页榜单及类目筛选。Python 销售 consumer 显式绑定第二个预览 Django 端口，使用同轮随机签名密钥；不得回退默认生产端口。大范围超限与恢复交互由 `tools/verify-market-query-ui.mjs` 的独立合成 API 验证。

AI 对话另含一段“广东仓库存复盘 · 合成演示”长回复，展示 Markdown 表格、底部输入区和配置版本；它不来自真实模型或真实经营数据。配置设置可读取系统默认并编辑页面草稿，持久保存、恢复与模型调用仍受下方只读限制。
