...
这一片仍**没有可信 append API**。现有签名财务页在 Worker 的独立 TS 内部适配器中，尚未注册 AI Django writer 可以调用的 `business_collection` 工具或配置直接签名的 finance_reader 通道。若接受调用方上传一页，页 SHA 可由上传者自行重算，无法证明其来自真实财务来源。因此本模块不接收页作为可发布输入，不自动调用模型，也不把物理块或纯重建结果标为 `persistentEvidenceVerified=true`。在日来源具备同等持久采集和全部来源独立核验前，整份 v3 任务保持 `collecting/manual`，五 Agent 与 HTML/XLSX 入口继续关闭。
