...
0042 上下文不暴露票据 `requestDigest` 或独立来源根。前者必须由可信票据发行流程原样交给调用者，0052 会对真实票据再核；后者由 0052 数据库函数独立复核，调用层不从本地候选推断。当前仅 fake-driver 测试，未执行真实 sealer 登录、PG 业务、生产迁移、付费模型或原生文件验收。
