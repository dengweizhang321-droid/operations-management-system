...
Codex heartbeat `automation-5`（“数据工作流统一监控”）每天上海时间 14:25 使用一个 Agent 统一只读检查 11 条现行正式工作流，原每小时 heartbeat `ai` 已暂停。对每个 workflow ID，它仍只统计 `mode=trigger/webhook` 且已经终止为 `error/crashed` 的不同 execution；最近一次 `success` 会把连续失败数视为 0，`running/waiting` 不计入失败，也不会触发竞争处置。最新三个生产 execution 连续失败且中间没有成功时，统一 Agent 才执行根因分析；日常不创建或委派子 Agent。
...
验证码、风控、凭据/登录、账号或店铺身份、跨店、来源未开放、任务歧义、点击或提交结果未决、活动清单归属不明、owner 冲突和需人工确认的内容完整性错误，即使达到三次也只能由 AI 诊断并给出唯一人工动作，不能自动修改或重放业务。未达到三次且没有必须转人工的安全故障时，统一 heartbeat 保持静默。由于 AI 不再每小时重复唤醒，实时的 60 分钟重试仍完全由 n8n 共享错误工作流负责；14:25 之前的人工介入提示以工作流既有失败关闭与通知能力为准。
