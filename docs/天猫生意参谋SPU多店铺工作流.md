...
Cookie 原文件路径仍通过 `TMALL_SYCM_COOKIE_FILE` 环境变量提供，或作为单独一行写入 Git 已忽略的 `.runtime/tmall-yijiu-sycm-cookie-path.txt`。指针文件只能保存路径，不能复制 Cookie 内容、账号或密码。辅助进程只监听本机环回地址，n8n 的 M、A、B、C、P 节点会把同一个 `$execution.id` 放入 `X-TERUISI-N8N-EXECUTION-ID` 请求头；helper 只允许 M 首次领取，后续节点必须同时匹配执行 ID 与当前阶段，旧执行、缺失 ID、乱序或并发请求均失败关闭。M、A 或 B 成功后若两分钟内没有下一节点领取，helper 会自动退出并由本地 Worker 启动器重新待命；C 完成后仍只保留 30 秒兼容窗口等待 P。工作流页面每 5 秒读取一次只含阶段、忙碌状态和 Cookie 文件是否存在的 `/health`，不会读取文件路径或 Cookie 内容；“辅助服务已就绪”只表示环回服务与 Cookie 原文件可用，Cookie 身份、千牛和阿里妈妈登录态仍在对应阶段重新核验。当前五段式副本必须从 M 开始，旧 A 起点不再进入同一状态机。
...
4. B 节点仅在计划非空时读取 Cookie，先校验 `sn` 登录身份必须属于亿玖店，再对当前业务日单独请求一次生意参谋导出。响应必须通过老式 XLS 魔数、25 MB 上限、当前 SPU 表头、唯一业务日、行数和 SHA-256 校验；`NO_DATA_ROWS + MISSING_EXPECTED_DATES` 明确记为 `SOURCE_NOT_READY` 并等待下一轮，不能当成损坏文件或合法零数据日。空计划直接生成受控空 manifest，不因无关的 Cookie 过期阻断待命链路。
5. C 节点再次核验文件路径、标准文件名、大小和哈希，生成店铺独立签收单，调用当前 `tmall_product_daily` 导入接口，并要求 HTTP 状态、导入状态、来源、平台、店铺、数据集和日期全部匹配，且“文件预检行数 = 批次行数 = 解析行数 = 落库回查行数”。任一不一致或同店同日覆盖未命中均视为失败。
