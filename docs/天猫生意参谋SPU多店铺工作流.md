...
7. P 点击“确定”后等待页面出现同时包含“离线数据生成成功”和“下载任务管理”的成功提示，只点击该提示内唯一的“立即前往/前往下载”；广告、普通活动入口或缺少成功语境的同名操作均拒绝点击。成功提示可在当前页跳转或打开新页，目标必须是用户指定的下载任务地址 `https://one.alimama.com/index.html?spm=a21dvs.28711903.0.d53b8f72b.7a7362ddGBIZmC#!/report/download-list`；只有恢复清单已经处于报表提交阶段、成功提示已消失时，才直接进入该受控地址，避免重复创建报表。下载任务管理每 8 秒刷新，最多等待 10 分钟。只有日期范围完全一致、文件名创建时间属于本轮、状态精确为“生成成功/已完成”且只有一个下载动作的任务可以下载；同一创建时间存在多个候选、历史任务、生成失败或超时均安全停止。
