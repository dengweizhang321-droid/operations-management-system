...
1. M 节点位于末段，且不再因为当天已有完成快照而预先跳过。它使用注册表中的亿玖店独立 Chromium profile 打开千牛 `商品 > 出售中`，从页面可见文本核验店铺身份，关闭右下角“重要通知”（仅在显示时），再从右下角打开“商品管家”。脚本只向右侧商品管家聊天输入“导出全部商品”、发送并确认，等待本轮唯一的新“前往下载”链接后保存 `.xlsx`；不再点击“批量导出表格”或“更多批量操作”。文件必须位于该店独立下载目录，并通过“发布模板”工作表、商品/SKU 表头、25 MB 上限、行数和 SHA-256 校验；之后始终调用当前导入接口。只有业务范围与规范化完整内容都一致时接口才返回 `duplicate`，否则创建新批次并核对快照日期、店铺、数据集和落库行数。
...
3. 拓丰店是 M 节点替代路径的单店试点。其注册项显式设置 `productMasterExportMode=on_sale_pagewise_excel`，n8n 仍调用同一 `/product-master` 终态接口并保持 `A→B→C→P→M`、execution owner、店铺键和浏览器关闭契约不变。helper 从“商品 > 我的商品 > 出售中”同时读回商品总数与“当前页/总页数”，要求总页数严格等于按每页 20 条计算的结果；每页先点击“商品标题”全选框并精确读回 `已选 N`，再选择“更多批量操作 > excel商品批量导出”。成功弹窗出现前清单保持 `page_export_submitting`，该状态说明业务点击结果未决，禁止自动重发；出现“商品批量导出任务创建成功”后才把页码、选中数和提交时间写为已提交。非最后一页只点“取消”关闭弹窗并点击右上角下一页，最后一页才点“前往下载”。导出记录必须在本轮首尾提交时间窗内恰好出现总页数条任务，多一条视为人工或并发任务混入，少一条或仍处理中则继续有界等待，明确失败则停止。所有任务逐个下载到拓丰独立目录，每份文件的唯一商品数必须等于对应页选中数；随后合并为一个“发布模板”工作簿，并要求无跨页商品重复、合并唯一商品数等于出售中总数、合并行数等于分页行数合计。只有该合并文件调用一次导入接口并完成批次、行数、店铺和快照回查，不能逐页导入。检测到原商品管家活动清单时先停止并要求人工核对，不能直接跨模式创建新任务。
...
6. P 节点只在 C 成功后核验当前目标日已有商品日覆盖；这是推广执行的前置条件，不是重复判断。目标日期按升序拆成独立日报计划，不会因为已经存在推广日期覆盖而预先跳过；n8n 每轮只处理 1 个业务日，直接运行 CLI 时仍有最多 30 日的有界上限。缺少目标日商品日覆盖时返回 `waiting_product_daily` 并等待后续重试。每个计划的开始日期和结束日期必须是同一天，禁止把连续日期合并成区间报表。它使用同一店铺独立 Chrome profile，从千牛店铺后台出售中页面复核可见店铺身份后，固定从页面最左侧点击“推广”，进入推广中心后点击左侧“货品全站推”，再点击全站推页面左侧“报表”和右侧“下载报表”；不得从阿里妈妈首页直接寻找推广入口。随后为当前业务日选择同日起止日期、选择“全部数据指标”并回读其选中状态，无法确认时停止。网页浮层只点击可安全识别的“关闭/忽略/暂不/稍后/我知道了/×”；原生对话框也只处理明确允许的信息提示，未知确认框、输入框或其他文案会停止本轮，明确拒绝“去优化/立即处理/立即报名/查看详情”等业务动作。
...
8. 每个推广日报先进入当前店铺下载目录内的独立临时目录，只接受本轮唯一 `.zip`，再原子移动到带运行 ID 的规范文件名。导入前必须通过 ZIP 魔数、25 MB 上限、压缩包内唯一 CSV、“日期 + 主体ID + 花费”表头、主体类型为“商品”、唯一目标日、行数和 SHA-256 校验。随后以 `source=tmall_promotion`、受控店铺和同日起止日期调用导入接口，核对 `promotion_daily` 批次、状态、行数、日期、店铺和落库回查；只有当前业务日确认覆盖后才处理下一天，任意一天失败即停止后续日期。接口成功但推广日期覆盖未命中仍视为失败；重跑仍会执行目标日，并由导入接口按完整业务内容决定 `duplicate` 或精确替换。
