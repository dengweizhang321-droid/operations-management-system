...
1. M 节点先查询 `tmall_product_master` 最新完成批次；当天快照已存在时直接跳过。否则使用注册表中的亿玖店独立 Chrome profile 打开千牛 `商品 > 出售中`，从页面可见文本核验店铺身份，关闭右下角“重要通知”（仅在显示时），再从左下角打开“商品管家”。脚本只向右侧商品管家聊天输入“导出全部商品”、发送并确认，等待本轮唯一的新“前往下载”链接后保存 `.xlsx`；不再点击“批量导出表格”或“更多批量操作”。文件必须位于该店独立下载目录，并通过“发布模板”工作表、商品/SKU 表头、25 MB 上限、行数和 SHA-256 校验；之后调用当前导入接口并核对精确批次、快照日期、店铺、数据集和落库行数。
2. M 节点在发送聊天指令前写入活动清单，并记录通知处理状态与商品管家入口状态；下载完成后记录路径、大小、哈希和行数。若“重要通知”出现但无法唯一定位关闭按钮，或左下角“商品管家”无法唯一定位，脚本会在发送指令前停止。若导入阶段失败，重跑只接管与清单完全一致的文件；若上次已经发送或确认但尚未唯一下载，则停止并要求人工核对右侧聊天，避免重复创建远端任务。登录失效返回 `waiting_login`，页面店铺名不符返回 `shop_identity_mismatch`，两者都不会继续导入。
