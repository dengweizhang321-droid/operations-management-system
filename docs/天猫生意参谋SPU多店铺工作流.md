...
1. M 节点先查询 `tmall_product_master` 最新完成批次；当天快照已存在时直接跳过。否则使用注册表中的亿玖店独立 Chrome profile 打开千牛 `商品 > 出售中`，从页面可见文本核验店铺身份，在右侧聊天输入“导出全部商品”、发送并确认，等待本轮唯一的新“前往下载”链接后保存 `.xlsx`。文件必须位于该店独立下载目录，并通过“发布模板”工作表、商品/SKU 表头、25 MB 上限、行数和 SHA-256 校验；之后调用当前导入接口并核对精确批次、快照日期、店铺、数据集和落库行数。
2. M 节点在发送聊天指令前写入活动清单，下载完成后记录路径、大小、哈希和行数。若导入阶段失败，重跑只接管与清单完全一致的文件；若上次已经发送或确认但尚未唯一下载，则停止并要求人工核对右侧聊天，避免重复创建远端任务。登录失效返回 `waiting_login`，页面店铺名不符返回 `shop_identity_mismatch`，两者都不会继续导入。
...
工作流保持 `active=false`。首次执行 M 前，必须在注册表指定的独立 Chrome profile 中人工完成千牛登录并确认“出售中”页面能看到亿玖店身份；脚本不会填写或保存密码。当前一次性辅助进程只用于人工触发的一轮执行；若要发布小时级定时触发，必须先把辅助进程纳入受控服务生命周期和健康检查，不能让 n8n 在辅助进程未运行时空触发。M 每个上海自然日最多生成一个已确认货品快照，后续小时补跑会跳过；Cookie 失效、身份不一致或接口返回非 XLS 时只更新原 Cookie 文件并重新启动辅助进程，不得把 Cookie 或登录密码粘贴进 n8n 节点。
