...
6. P 节点只在 C 成功后查询 `天猫 + 当前受控店铺 + promotion_daily` 与商品日覆盖，选择最早一段连续推广缺口（按页面已确认的“过去 30 天”能力，单轮最多 30 天）。它使用同一店铺独立 Chrome profile，从千牛店铺后台出售中页面复核可见店铺身份后，固定从页面最左侧点击“推广”，进入推广中心后点击左侧“货品全站推”，再点击全站推页面左侧“报表”和右侧“下载报表”；不得从阿里妈妈首页直接寻找推广入口。随后选择计划起止日期、勾选“全部数据指标”并确认。过程中出现平台消息、通知或广告浮层时，只点击可安全识别的“关闭/忽略/暂不/稍后/我知道了/×”，明确拒绝“去优化/立即处理/立即报名/查看详情”等业务动作。
7. P 点击“确定”后等待页面右上角出现同时包含“离线数据生成成功”和“下载任务管理”的成功提示，只点击该提示内唯一的“立即前往/前往下载”；广告、普通活动入口或缺少成功语境的同名操作均拒绝点击。成功提示可在当前页跳转或打开新页，目标必须是用户指定的下载任务地址 `https://one.alimama.com/index.html?spm=a21dvs.28711903.0.d53b8f72b.7a7362ddGBIZmC#!/report/download-list`；进入新页面后按本轮日期和创建时间下载对应任务。只有恢复清单已经处于报表提交阶段、成功提示已消失时，才直接进入该受控地址，避免重复创建报表。下载任务管理每 8 秒刷新，最多等待 10 分钟。只有日期范围完全一致、文件名创建时间属于本轮、状态精确为“生成成功/已完成”且只有一个下载动作的任务可以下载；同一创建时间存在多个候选、历史任务、生成失败或超时均安全停止。
