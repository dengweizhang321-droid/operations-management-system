...
1. M 节点位于末段，且不再因为当天已有完成快照而预先跳过。丽力、亿用和炊之王继续使用商品管家模式：从各自独立 Chromium profile 打开千牛 `商品 > 出售中`，核验店铺身份后发送“导出全部商品”，只接管本轮唯一任务并校验、导入、回查。亿玖、拓丰和马思图使用第 3 项的逐页模式。两种模式都必须把文件限制在当前店独立下载目录，并通过发布模板、商品/SKU 表头、25 MB 上限、行数和 SHA-256 校验；只有业务范围与规范化完整内容都一致时接口才返回 `duplicate`。
