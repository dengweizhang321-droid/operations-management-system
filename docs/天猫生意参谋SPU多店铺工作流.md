...
2. M 节点在发送聊天指令前写入活动清单，并记录通知处理状态与商品管家入口状态；下载完成后记录路径、大小、哈希和行数。右下角“商品管家”既可以是可见文字按钮，也可以是通过 `aria-label`、`title`、图标 `alt` 或商品管家专用类名标识的入口，但仍须通过右下角位置、尺寸和唯一候选校验。若“重要通知”出现但无法唯一定位关闭按钮，或“商品管家”无法唯一定位，脚本会在发送指令前停止。若导入阶段失败，重跑只接管与清单完全一致的文件；若上次已经发送或确认但尚未唯一下载，则停止并要求人工核对右侧聊天，避免重复创建远端任务。登录失效返回 `waiting_login`，页面店铺名不符返回 `shop_identity_mismatch`，两者都不会继续导入。
