...
此次先完成1份清理及后验，再完成11份；之后仅停止本任务的只读候选校验进程，设置清理进程专用 `UV_THREADPOOL_SIZE=32`，从完整云端预检重新开始并完成其余125份。所有检查项目和固定清理范围不变。此前本任务并行只读检查的命令行曾触发候选路径引用保护，退出该检查进程后才完整重试；失败审计保留。工具与文档仍位于未合入main的 `codex/storage-retention-cloud-archive` 分支，工作树继续保留。历史版本中这137个依赖目录缺失是已验证的归档状态，不得批量重新安装；单个恢复仍须遵守下方原始摘要和受控路径流程。此前被自动审批阻止删除的D盘旧验证临时副本不在本次已清理范围。
