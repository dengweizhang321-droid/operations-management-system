...
每页重算规范 UTF-8 原文字节数和 SHA-256、原文行数、固定推广行形状与日期、工具请求参数摘要、`PageReconciler` 游标与控制总额、收据链；对照 0036 段的进度原文、进度摘要与段证明摘要。跨段恢复要求上一段候选的任务、来源根、版本、密钥、段序号与摘要吻合。候选摘要只是完整性自检，**可由持有候选文本的人重算**，后续必须由数据库不可变回执或受保护签名将每段确认结果持久化，才可用于最终封存。
