...
2026-09-17 第十六批已实现 v2 纯目录合同、单一目录表及多卷拆分/渲染底座；第十七批接通显式 v2 API 的创建、采集、恢复、封存及有界读取，工作台可查看和控制。第十八批接入轻量报告引用、独立 Agent 工具、每个 Agent 的目录回执核验及三专业节点并行。第十九批接入共享封存读取、内部预算计算和全部表到临时多卷文件的适配。首次工作台仍创建 v1；v2 报告固定预算及持久多卷下载尚未接入，因此工作台暂不开放 v2 分析启动。目录支持 48 项也不代表事实容量提高。实际进度及验证见实施说明中的第十六至十九批；下文未明确实现的部分仍是设计。
...
独立审查确认不能只把纯多卷 renderer 接到旧任务：旧 chunk 唯一键没有卷号、数据库 ready 门禁固定一对 HTML/XLSX，完整追溯清单也可能超过 run 的 128 KiB。拟增加 renderer 4 与独立不可变卷分片表；卷 1—100 保存 HTML/XLSX，卷 0 保存完整 JSON 清单。旧 renderer 1/2/3 和旧分片表不回填或改写。新表需同步权限、就绪、备份与逆迁移拒绝条件，实际迁移号以实施时最新编号为准。

根记录只存不超过 128 KiB 的紧凑目录、绑定、attempt、卷数、各文件字节/摘要/分片数及完整清单摘要。详细表片段、行区间和源表证明保存为另有明确字节上限的 JSON 文件。单文件 256 MiB、任务 1 GiB、用户 2 GiB、全局 8 GiB 不自动增加；旧 attempt 的分片继续占用额度。所有临时输出也须设置整批高水位，防止多个合格文件合计耗尽磁盘。v4 单独调整 TableSpool 的逻辑表行数限制，旧版每表 100 万行门禁保持。
