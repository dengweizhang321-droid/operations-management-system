...
本节保留设计依据；第二十二批完成参数底座，第二十三批完成独立执行协议、三个工具、逐 Agent 读取证明、实际容量预检及预算文件交付。报告 API 已接受合法 v2 固定预算，首次工作台预算入口尚待接入。`budget.normalize()` 允许最多 100 个目标、规范 JSON 最多 **48000 UTF-8 字节**，报告快照继续为 **32768 字节**，因此参数存于独立不可变记录而非内联 snapshot。现有 v1/v2 snapshot 原始字节、摘要和旧工具目录保持不变，不回填参数引用，不修改已应用迁移。以下建议值已由候选代码及隔离边界验证采用，不代表生产启用；参数可存储也不等于模型上下文可容纳。
