...
2026-09-17 第十六批已实现 v2 纯目录合同、单一目录表及多卷拆分/渲染底座；第十七批接通显式 v2 API 的创建、采集、恢复、封存及有界读取，工作台可查看和控制。第十八批接入轻量报告引用、独立 Agent 工具、每个 Agent 的目录回执核验及三专业节点并行。第十九批接入共享封存读取、内部预算计算和全部表到临时多卷文件的适配；第二十批接通持久卷任务、整批恢复与逐卷校验下载。首次工作台仍创建 v1，v2 固定预算尚未接入，v2 分析按钮仍待后续工作台规划接线。目录支持 48 项也不代表事实容量提高。实际进度及验证见实施说明中的第十六至二十批；下文未明确实现的部分仍是设计。
...
### H. v2 固定预算引用与独立执行协议（尚未实施）

本节是后续开发设计，不代表现有 v2 报告已经支持固定预算。当前 `business_reports._create_v2()` 明确拒绝预算；`budget.normalize()` 允许最多 100 个目标、规范 JSON 最多 **48000 UTF-8 字节**，而 `0012_report_library` 的报告快照数据库上限为 **32768 字节**。将完整预算参数直接放入 snapshot 会使合法参数无法保存，轻量 workflow input 也不能解决这一冲突。保留现有 v1/v2 snapshot 原始字节、摘要、32768 字节门禁及旧工具目录，不回填参数引用，不修改已应用迁移。
