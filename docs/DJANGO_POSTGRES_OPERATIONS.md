...
本文定义本机 Django/PostgreSQL 共享权威库的日常逻辑备份、完整性复验、隔离恢复演练和保留策略。它补充各业务域的一次性切换备份，不改变销售、财务、网店、市场、商品经营和运营事务新品项目子域的正式单写架构，也不提供 D1 回退入口。
...
- 备份使用 PostgreSQL exported snapshot，把表行数、Django migrations、动态 revision 和写入权威证据与同一个 `pg_dump` 快照绑定。证据必须覆盖库中全部销售、ERP、财务、网店、市场、商品经营和已经安装的库存、补货、运营事务新品表，并绑定各已迁移域或候选迁移域的 revision、迁移 run 和 authority；不存在对应结构的历史备份仍按旧证据读取。authority 尚未激活的候选域只能表达候选/影子状态，不能声称已切换。备份期间的新写入不会造成半新半旧的归档。
...
4. 恢复后的全部 `sales_*`、`erp_*`、`finance_*`、存在时的 `netshop_*`、`market_*`、`product_*`、`inventory_*`、`replenishment_*`、`workflow_*` 和 `django_migrations` 行数，migration 清单、revision 与 write authority 的内容摘要与备份快照完全相同；
5. 生产数据库和在线服务状态均未改变。
