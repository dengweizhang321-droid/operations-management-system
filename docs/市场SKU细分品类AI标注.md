...
1. 依次应用 `drizzle/0016_market_sku_annotations.sql`、`drizzle/0017_market_annotation_reliability.sql`、`drizzle/0054_market_annotation_concurrency_settings.sql` 与 `drizzle/0055_market_annotation_throughput.sql`。
...
3. 在独立的并发卡中设置模型并发数，再点击“开始/恢复云端识别”。并发范围为 1–50，按“三级类目 + 执行器”记忆；云端默认 10、建议 10–20，本地 Ollama 默认且建议 1。已有任务可在“当前任务并发”中直接修改，运行中点击“保存并应用”也会即时更新。每个浏览器请求仍只处理一张图片；同一任务内按“类目 + scope + SKU + 榜单维度 + 图片哈希”领取唯一推理单元，跨月同图只推理一次并将结果批量写回全部月份。浏览器关闭后可继续，重复创建同一业务任务会恢复已有任务；单次模型或网络异常从 5 秒短冷却和温和降并发开始，同一冷却窗口不会因多路同时失败直接跌到 1，重复异常才继续阶梯降级；供应商限流仍按减半并发与有界退避处理。连接每连续成功 3 张增加一路，直至恢复该类目的配置值。运行期间页面使用轻量进度接口，结束后才加载复核明细。
