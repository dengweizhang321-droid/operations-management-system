...
3. 在独立的并发卡中设置模型并发数，再点击“开始/恢复云端识别”。并发范围为 1–50，按“三级类目 + 执行器”记忆；云端默认 10、建议 10–20，本地 Ollama 默认且建议 1。已有任务可在“当前任务并发”中直接修改，运行中点击“保存并应用”也会即时更新。每个浏览器请求仍只处理一张图片，每条 SKU 独立领取 claim、完成后写回 D1。浏览器关闭后可继续，重复执行不会重复创建候选；单次模型或网络异常从 5 秒短冷却和温和降并发开始，同一冷却窗口不会因多路同时失败直接跌到 1，重复异常才继续阶梯降级；供应商限流仍按减半并发与有界退避处理。连接每连续成功 3 张增加一路，直至恢复该类目的配置值。
