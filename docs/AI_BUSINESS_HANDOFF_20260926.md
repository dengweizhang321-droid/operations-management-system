...
4. 仍需当前京东交易概况导出的店铺 UV 字段/身份及区间去重来源、实际模型选择和权威人民币费用上限。缺项保持 `unknown` 或关闭，不用商品访客、日 UV 求和或静态 XLSX 检查替代。本机 Excel 16.0 COM 已通过不落盘的合成 `SUM` 初值/改输入重算，进程退出；真实系统工作簿原生打开和公式验收仍未完成。
