...
4. 仍需当前京东交易概况导出的店铺 UV 字段/身份及区间去重来源、实际模型选择和权威人民币费用上限。缺项保持 `unknown` 或关闭，不用商品访客、日 UV 求和或静态 XLSX 检查替代。本机 Excel 16.0 COM 已通过不落盘的合成 `SUM` 初值/改输入重算，并只读打开系统生成的 4.3 MB 两表工作簿及原参考 42.7 MB/30 表工作簿，均正常退出。新版本真实系统工作簿原生公式/性能验收仍未完成。
