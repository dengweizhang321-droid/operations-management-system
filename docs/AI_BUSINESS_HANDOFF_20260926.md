...
- `0073` 未建迁移：0062 的原暂停报告与 0064 的另立合成五角色报告不能合并成同 job/provider 已读。五角色逐一拒绝 0062 回执、拥有方数据库重读服务拒绝空/合成执行的隔离 PG 2 项通过，证据 `.runtime/ai-pg-2853609f92c2/tests.log`。不把合成链冒充真实 Agent 已读。
- 测试专用**第二全新 PostgreSQL 集群**在合成超级用户/随机合成密钥下保留 owner/ACL，恢复 12 受保护角色、8 表与 1 合成密钥；0068–0072 目录逐项通过，故意所有者和函数授权漂移均拒绝回滚。证据 `E:\codex-artifacts\ai-business-trial-acceptance-20260925\archived-pg\ai-pg-38e31f193b17-protected-audit\protected-cross-cluster\evidence.json`。正式备份/非超级用户迁移/归档加密均未因此通过。
