...
历史财务只读响应在精确 ERP 别名店铺选择下显示 `hasData=true`、选定 `2026-08`。这只是旧 API 的历史快照，不是 v4 完整财务拥有方页、工具收据或段证明。若当前该店真实财务来源可读，应按既有一财务加推广的 v4 合同保留其自然月背景；若取不到，不能构造合成空月满足 0038，应保持封存关闭并另设计版本化推广专用 profile。
