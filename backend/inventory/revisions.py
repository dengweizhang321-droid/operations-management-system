...
from .models import InventoryDataRevision


def revision_value(*, for_update: bool = False) -> str:
    query = InventoryDataRevision.objects
    if for_update:
        query = query.select_for_update()
    row = query.get(domain="inventory")
...
def bump_revision(reason: object) -> str:
    with transaction.atomic():
        row = InventoryDataRevision.objects.select_for_update().get(domain="inventory")
        payload = json.dumps(
