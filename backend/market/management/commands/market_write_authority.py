...
def _authority(source: sqlite3.Connection) -> dict[str, object]:
    ...
    if row is None:
        raise CommandError("D1 市场 authority 尚未安装。")
    return dict(row)
...
def _source_receipt(
    ...
    _validate_source(source, apply=False, allowed_owners=allowed_owners)
    context = _build_context(source)
    counts, digests, source_digest = _snapshot(_source_sections(source, context))
    return counts, digests, source_digest, f"market-{source_digest[:24]}"


def _assert_postgres_quiet() -> MarketWriteAuthority:
    authority = MarketWriteAuthority.objects.select_for_update().filter(id=1).first()
    control = MarketNetshopProjectionControl.objects.select_for_update().filter(id=1).first()
    if authority is None or control is None:
        raise CommandError("PostgreSQL 市场 authority 或投影控制行尚未初始化。")
    if (
        MarketWriteRequestReceipt.objects.filter(status="processing").exists()
        or MarketImportAttempt.objects.filter(outcome="validating").exists()
        or MarketImportScopeHead.objects.exclude(status="ready").exists()
        or MarketImportScopeHead.objects.exclude(owner_token="").exists()
        or MarketImageCacheClaim.objects.exists()
        or MarketImageCacheJob.objects.exclude(lease_token_hash="").exists()
        or MarketBrandRecognitionJob.objects.exclude(lease_token_hash="").exists()
        or MarketAnnotationItem.objects.filter(status__in=["claimed", "inferencing"]).exists()
        or MarketAnnotationItem.objects.exclude(lease_token_hash="").exists()
        or MarketAnnotationValidationResult.objects.exclude(claim_token_hash="").exists()
        or MarketAnnotationCloudRun.objects.exclude(lease_token_hash="").exists()
        or MarketDownloadTask.objects.exclude(execution_token_hash="").exists()
        or bool(control.syncing_revision)
        or bool(control.owner_token_hash)
        or bool(control.syncing_owner)
        or control.syncing_total != 0
        or control.syncing_offset != 0
    ):
        raise CommandError("PostgreSQL 市场写入状态不静默，拒绝 authority 变更。")
    return authority
...
def _verified_migration(
    *,
    ...
    run = MarketMigrationRun.objects.filter(
    ...
    if (
        ...
        raise CommandError("approved-run-id 不是当前 D1 快照的完成市场迁移。")
    target_counts, target_digests, target_digest = _snapshot(_target_sections())
    revision = MarketDataRevision.objects.filter(domain="market").first()
    if (
        ...
        raise CommandError("PostgreSQL 市场事实已偏离获批迁移快照。")
    return run


class Command(BaseCommand):
    help = "Prepare, abort, activate, or inspect the market single-write authority."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        if (
            ...
            raise CommandError("生产市场 authority 只能由 migration_writer 进程角色操作。")
        path = Path(str(options["source"])).expanduser().resolve()
        if not path.is_file():
            raise CommandError("D1 市场源文件不存在。")
        ...
        try:
            ...
            target = MarketWriteAuthority.objects.filter(id=1).first()
            if not mutating:
                source.rollback()
                self.stdout.write(
                    json.dumps(
            ...
            if approved != run_id:
                raise CommandError("approved-run-id 与当前冻结市场事实不一致。")

            if prepare:
                with transaction.atomic():
                    target = _assert_postgres_quiet()
                    if target.status != "d1":
                        raise CommandError("PostgreSQL 市场 authority 已不在 d1。")
                if current["owner"] == "pending" and current["cutover_id"] == cutover_id:
                    source.rollback()
                elif current["owner"] == "d1":
                    changed = source.execute(
                        "UPDATE market_write_authority SET owner='pending',epoch=epoch+1,"
                        "cutover_id=?,updated_at=CURRENT_TIMESTAMP "
                        "WHERE id=1 AND owner='d1'",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 市场 authority prepare 未取得唯一所有权。")
                    source.commit()
                else:
                    raise CommandError("D1 市场 authority 无法进入本次 pending。")
                result = {
                    "status": "prepared",
                ...
                with transaction.atomic():
                    target = _assert_postgres_quiet()
                    if target.status != "d1":
                        raise CommandError("PostgreSQL 已取得市场写权，不能回退 pending。")
                if current["cutover_id"] != cutover_id:
                    raise CommandError("D1 不在本次市场 cutover 的 pending 状态。")
                if current["owner"] == "d1":
                    source.rollback()
                else:
                    changed = source.execute(
                        "UPDATE market_write_authority SET owner='d1',epoch=epoch+1,"
                        "updated_at=CURRENT_TIMESTAMP "
                        "WHERE id=1 AND owner='pending' AND cutover_id=?",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 市场 authority pending 回退失败。")
                    source.commit()
                result = {
                    "status": "aborted",
                    "cutoverId": cutover_id,
                ...
                if current["cutover_id"] != cutover_id:
                    raise CommandError("D1 必须先进入本次市场 cutover 的 pending 状态。")
                _verified_migration(
                    run_id=run_id,
                ...
                with transaction.atomic():
                    ...
                    if target.status == "d1":
                        ...
                        raise CommandError("PostgreSQL 市场 authority 与本次 cutover 冲突。")
                    epoch = str(target.authority_epoch)
                if current["owner"] == "pending":
                    changed = source.execute(
                        "UPDATE market_write_authority SET owner='postgresql',epoch=epoch+1,"
                        "updated_at=CURRENT_TIMESTAMP "
                        "WHERE id=1 AND owner='pending' AND cutover_id=?",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError(
                            "D1 市场 authority 终态写入失败；PostgreSQL 已安全持有写权。"
                        )
                    source.commit()
                else:
                    source.rollback()
            ...
            source.rollback()
            raise CommandError("D1 市场 authority 操作失败。") from error
        finally:
            source.close()
