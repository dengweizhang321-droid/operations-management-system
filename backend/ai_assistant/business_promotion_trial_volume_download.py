"""Bounded immutable renderer-9 chunk read after internal publication.
...
def _fence(row, principal, compact):
    ...
    if row.renderer_version != 9 or row.draft or row.status != "ready":
...
def chunk(row, index, kind, sequence, principal):
    ...
    try:
        compact = volume_delivery.validate(json.loads(row.manifest_json),
            ...
            draft=False, renderer_version=9)
