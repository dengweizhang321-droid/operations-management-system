...
def verify_seal(run_id, principal):
    ...
    with transaction.atomic():
        # No current-revision equality requirement: a sealed snapshot may be
        # historical after later legal imports. This probe supplies the fixed
        # migration cutoff and checks that source write fences remain enabled.
        ...
        try:
            ...
            raise AiError("v4封存来源段链或正文无法重建", "conflict", 409) from error
        ...
        if (final_actor != actor or final_parent.version != parent.version
                ...
                or validation._key()[1] != key_id or time.monotonic() > deadline):
        ...
        result = {"schemaVersion": SCHEMA, "runId": parent.id,
            ...
            "sourceRefs": [{"sourceKey": item.source_key,
                "sourceRef": item.source_ref, "sourceRevision": item.source_revision}
                for item in sources],
