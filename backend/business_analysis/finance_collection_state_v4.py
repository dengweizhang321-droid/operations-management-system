"""Pure v4 verification of complete signed monthly finance page sequences.

Versioned from v3 without changing its 1,999-page/64-MiB contract. A checkpoint
is a bounded summary, not owning authorization or durable evidence by itself.
"""
...
from . import evidence_v3, evidence_v4, finance_source
from .contracts import AnalysisContractError, MAX_SAFE_INTEGER, canonical, comparison_periods, digest

SCHEMA = "business-v4-finance-collection-checkpoint-v1"
RESULT_SCHEMA = "business-v4-finance-collection-result-v1"
PAGE_SCHEMA = "business-finance-owned-page-v1"
PAGE_BYTES = 38_000
CHECKPOINT_BYTES = 32_768
TOTAL_BYTES = evidence_v4.MAX_SOURCE_BYTES
MAX_DATA_PAGES = evidence_v4.MAX_SOURCE_PAGES
ZERO = "0" * 64
...
def consume(checkpoint, page, *, trusted_query):
    ...
    _require(result["pageCount"] <= MAX_DATA_PAGES
             and result["storedBytes"] <= TOTAL_BYTES,
             "财报来源超过完整持久容量，不得截断完成")
