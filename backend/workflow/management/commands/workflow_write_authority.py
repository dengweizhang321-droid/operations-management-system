...
from .migrate_workflow_launch_from_d1 import (
    GENERATION_VERSION,
    _counts,
    _sha,
    _source_snapshot,
...
def _source_authority(connection: sqlite3.Connection) -> dict[str, object]:
    row = connection.execute(
        ...
        "FROM workflow_launch_write_authority WHERE id=1"
    ).fetchone()
    if row is None:
        raise CommandError("D1 新品 authority 尚未安装")
    return dict(row)


def _verified_run(source_digest: str, counts: dict[str, int]) -> WorkflowMigrationRun:
    run = WorkflowMigrationRun.objects.filter(
        id=f"workflow-{source_digest[:32]}",
        mode="apply",
        status="verified",
    ...
    if (
        ...
        or _sha(target) != source_digest or _counts(target) != counts
    ):
        raise CommandError("当前 D1 新品事实没有可激活的已复验迁移")
    return run


def _assert_postgres_quiet() -> WorkflowWriteAuthority:
    authority = WorkflowWriteAuthority.objects.select_for_update().filter(id=1).first()
    if authority is None:
        raise CommandError("PostgreSQL 新品 authority 尚未初始化")
    if WorkflowWriteRequestReceipt.objects.filter(status="processing").exists():
        raise CommandError("PostgreSQL 新品仍有处理中写请求")
    return authority


class Command(BaseCommand):
    help = "Prepare, abort, activate, or inspect the structured workflow authority."
    ...
    def handle(self, *args, **options) -> None:
        ...
        try:
            ...
            target = WorkflowWriteAuthority.objects.filter(id=1).first()
            if not mutating:
                source.rollback()
                self.stdout.write(json.dumps({
                    "status": "ok",
                    "d1": {"owner": current["owner"], "epoch": int(current["epoch"]), "cutoverId": current["cutover_id"]},
                    "postgresql": {
            ...
            if prepare:
                ...
                if current["cutover_id"] != cutover_id or current["owner"] not in {"pending", "postgresql"}:
                    raise CommandError("D1 必须先进入本次新品 cutover 的 pending 状态")
                with transaction.atomic():
                    target = _assert_postgres_quiet()
                    if target.status == "disabled":
                        ...
                        raise CommandError("PostgreSQL 新品 authority 与本次 cutover 冲突")
                    epoch = str(target.authority_epoch)
                if current["owner"] == "pending":
                    changed = source.execute(
                        "UPDATE workflow_launch_write_authority SET owner='postgresql',epoch=epoch+1,"
                        "updated_at=CURRENT_TIMESTAMP WHERE id=1 AND owner='pending' AND cutover_id=?",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 新品 authority 终态写入失败；PostgreSQL 已安全持有写权")
                    source.commit()
                else:
                    source.rollback()
            ...
            source.rollback()
            raise CommandError("D1 新品 authority 操作失败") from error
        finally:
            source.close()
