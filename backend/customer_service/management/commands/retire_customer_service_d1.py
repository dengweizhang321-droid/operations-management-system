...
from .customer_service_write_authority import (
    CUTOVER_ID_RE,
    RUN_ID_RE,
...
from .migrate_customer_service_from_d1 import _counts, _sha, _source_snapshot


RETIREMENT_VERSION = "customer-service-domain-retirement-receipt-v1"
SMOKE_VERSION = "customer-service-system-test-receipt-v1"
R2_EVIDENCE_VERSION = "customer-service-r2-retirement-evidence-v1"
...
RETIREMENT_GUARDS = tuple(
    f"customer_service_retired_{surface}_{operation}_guard"
    for surface in (
        "fingerprints",
        "attempts",
        "heads",
...
def _load_smoke_receipt(
    ...
    value, payload = _load_json_file(path, "客服系统测试 receipt")
    ...
    if (
        ...
        raise CommandError("系统测试 receipt 未证明客服完整垂直链路通过。")
    _recent_timestamp(value.get("recordedAt"), "客服系统测试 receipt")
    return value, _sha256(payload)


def _load_r2_evidence(path: Path) -> tuple[dict[str, object], str]:
    value, payload = _load_json_file(path, "客服 R2 退役证据")
    if (
        ...
        raise CommandError("客服 R2 退役证据未证明旧共享上传前缀为空。")
    _recent_timestamp(value.get("recordedAt"), "客服 R2 退役证据")
    return value, _sha256(payload)
...
def _migration_path() -> Path:
    path = (
        ...
        / "drizzle"
        / "0108_customer_service_domain_retirement.sql"
    ).resolve()
    if not path.is_file():
        raise CommandError("缺少受控 D1 客服退役迁移。")
    return path
...
def _shared_receipt(source: sqlite3.Connection) -> dict[str, object]:
    sections = {
        "fingerprints_other": _query_rows(
            ...
            "WHERE domain<>'customer-service' ORDER BY sequence",
        ),
        ...
        "heads_other": _query_rows(
            ...
            "WHERE domain<>'customer-service' ORDER BY domain,scope_key",
        ),
        "uploads_other": _query_rows(
            source,
            "SELECT * FROM inventory_import_uploads "
            "WHERE fingerprint NOT LIKE 'customer-service:%' ORDER BY id",
        ),
        "upload_chunks_other": _query_rows(
            ...
            "WHERE u.fingerprint NOT LIKE 'customer-service:%' "
        ...
        "upload_results_other": _query_rows(
            ...
            "WHERE u.fingerprint NOT LIKE 'customer-service:%' ORDER BY r.upload_id",
        ),
        "retirement_receipts_other": _query_rows(
            source,
            "SELECT * FROM domain_retirement_receipts "
            "WHERE domain<>'customer-service' ORDER BY domain",
        )
        if source.execute(
...
def _legacy_upload_counts(source: sqlite3.Connection) -> dict[str, int]:
    sessions = int(
        source.execute(
            ...
            "WHERE fingerprint LIKE 'customer-service:%'"
        ).fetchone()[0]
    )
    active = int(
        source.execute(
    ...
    chunks = int(
        source.execute(
            ...
            "JOIN inventory_import_uploads u ON u.id=c.upload_id "
            "WHERE u.fingerprint LIKE 'customer-service:%'"
        ).fetchone()[0]
    )
    results = int(
        source.execute(
...
def _build_plan(
    ...
    *,
    ...
    d1_authority = source.execute(
        "SELECT owner,epoch,cutover_id FROM customer_service_write_authority WHERE id=1"
    ).fetchone()
    if (
        ...
        raise CommandError("D1 客服 authority 不属于本次已激活 cutover。")
    upload_counts = _legacy_upload_counts(source)
    if upload_counts["active"] or upload_counts["chunks"]:
        raise CommandError("D1 客服旧分片尚未完成或对象键尚未清零。")
    ...
    with transaction.atomic():
        ...
        if (
            ...
            raise CommandError("PostgreSQL 客服 authority 不属于本次完成迁移。")
...
def _completed_receipt(source: sqlite3.Connection) -> dict[str, object] | None:
    ...
    row = source.execute(
        ...
        "FROM domain_retirement_receipts WHERE domain='customer-service'"
    ).fetchone()
...
def _verify_retired(source: sqlite3.Connection, receipt: dict[str, object]) -> None:
    if (
        ...
        raise CommandError("D1 客服退役 receipt 无效。")
    views = {
    ...
    if set(views) != set(TOMBSTONE_VIEWS) or any(
        "customer-service-domain-retired-v1" not in sql for sql in views.values()
    ):
        raise CommandError("D1 客服 tombstone view 集合不完整。")
    for view in TOMBSTONE_VIEWS:
        if int(source.execute(f'SELECT COUNT(*) FROM "{view}"').fetchone()[0]) != 0:
            raise CommandError("D1 客服 tombstone view 非空。")
    guards = {
        str(row[0])
        for row in source.execute(
            "SELECT name FROM sqlite_master WHERE type='trigger' "
            "AND name LIKE 'customer_service_retired_%_guard'"
        )
    }
    if guards != set(RETIREMENT_GUARDS):
        raise CommandError("D1 客服永久 guard 集合不完整。")
    if _query_rows(
        source,
        "SELECT * FROM import_content_fingerprints WHERE domain='customer-service'",
    ) or _query_rows(
        source,
        "SELECT * FROM import_content_attempts WHERE domain='customer-service'",
    ) or _query_rows(
        source,
        "SELECT * FROM import_scope_heads WHERE domain='customer-service'",
    ):
        raise CommandError("D1 共享导入表仍含客服域行。")
    if _legacy_upload_counts(source) != {
        ...
        raise CommandError("D1 共享上传表仍含客服域行。")
...
def _statements(payload: bytes) -> list[str]:
    try:
        ...
        raise CommandError("D1 客服退役迁移不是 UTF-8。") from error
    statements = [
    ...
    if len(statements) < 70 or not statements[0].startswith(
        ...
        raise CommandError("D1 客服退役迁移结构不符合契约。")
    return statements
...
def _write_audit(path: Path, payload: dict[str, object]) -> None:
    ...
    if path.exists():
        raise CommandError("客服退役审计输出已存在，拒绝覆盖。")
...
class Command(BaseCommand):
    help = "Plan or execute terminal D1/R2 retirement after customer-service cutover."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        if (
            ...
            raise CommandError("生产 D1/R2 客服退役只能由 migration_writer 操作。")
        ...
        try:
            ...
            if completed is not None:
                ...
                if (
                    ...
                    raise CommandError("既有 D1 客服退役 receipt 与本次请求冲突。")
                source.rollback()
                result = {
            ...
            source.execute(
                ...
                "VALUES ('customer-service',?,'approved',?,?,?,?,?,?,?,?,?,NULL)",
                (
                    RETIREMENT_VERSION,
                    cutover_id,
                    approved_plan_id,
            ...
            if receipt is None:
                raise CommandError("D1 客服退役未生成完成 receipt。")
            _verify_retired(source, receipt)
            if _shared_receipt(source) != plan["shared"]:
                raise CommandError("D1 客服退役改变了其他域共享状态。")
            source.commit()
            result = {
                "status": "retired",
                "version": RETIREMENT_VERSION,
            ...
            source.rollback()
            raise CommandError("D1 客服退役事务失败。") from error
        finally:
            source.close()
