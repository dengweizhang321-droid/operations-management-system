...
login. The public read mode is now `django`, following explicit user confirmation
and successful online verification. Before stopping the local stack, first switch
the edge configuration back to `legacy` and verify the three public endpoints;
then run the runtime script with `-Action Stop`. See the
[migration guide](../docs/DJANGO_SALES_MIGRATION.md) for evidence, performance,
