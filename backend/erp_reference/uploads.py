...
from .models import ErpReferenceRawUploadChunk, ErpReferenceRawUploadSession
...
def _safe_file_name(value: object) -> str:
    ...
    if not name.lower().endswith(".xlsx"):
        raise _error("仅支持 .xlsx 格式的吉客云 ERP 报表", status=422)
    return name
...
def _session_payload(session: ErpReferenceRawUploadSession) -> dict[str, object]:
    indexes = list(session.chunks.order_by("chunk_index").values_list("chunk_index", flat=True))
    return {
        "id": str(session.id), "fingerprint": session.fingerprint,
        "source": session.source_key, "fileName": session.file_name,
        ...
        "receivedBytes": int(session.received_bytes), "status": session.status,
        "ownerGeneration": int(session.owner_generation),
        "expiresAt": session.expires_at.isoformat(),
    }


def _locked_session(upload_id: object, actor_email: str) -> ErpReferenceRawUploadSession:
    if not isinstance(upload_id, str) or not UUID_RE.fullmatch(upload_id):
        raise _error("上传会话标识无效")
    session = ErpReferenceRawUploadSession.objects.select_for_update().filter(id=upload_id).first()
...
def cleanup_expired(limit: int = 20) -> int:
    ids = list(
        ErpReferenceRawUploadSession.objects.filter(expires_at__lte=timezone.now())
        .order_by("expires_at").values_list("id", flat=True)[:limit]
    )
    if ids:
        ErpReferenceRawUploadSession.objects.filter(id__in=ids).delete()
    return len(ids)


def begin_upload(payload: dict[str, object], actor_email: str) -> dict[str, object]:
    if set(payload) != {"action", "source", "fileName", "fileSizeBytes", "chunkCount", "fingerprint"}:
        raise _error("上传初始化字段集合无效")
    source = payload.get("source")
    if source not in {"products", "combos"}:
    ...
    if not isinstance(fingerprint, str) or not fingerprint.strip() or len(fingerprint) > 255:
        raise _error("上传指纹无效")
    normalized = f"erp:{source}:{fingerprint.strip()}"
    with transaction.atomic():
        lock_active_authority()
        cleanup_expired()
        session = (
            ErpReferenceRawUploadSession.objects.select_for_update()
            .filter(
                fingerprint=normalized, actor_email=actor_email[:320],
                expires_at__gt=timezone.now(), status__in=["uploading", "ready", "processing", "completed"],
            ).order_by("-created_at").first()
        )
        if session is not None:
            if (
                session.source_key != source or session.file_name != file_name
                or int(session.file_size_bytes) != file_size or int(session.chunk_count) != chunk_count
            ):
                raise _error("上传指纹已绑定另一份文件", code="version_conflict", status=409)
            return {"upload": _session_payload(session)}
        session = ErpReferenceRawUploadSession.objects.create(
            fingerprint=normalized, source_key=str(source), actor_email=actor_email[:320],
            file_name=file_name, file_size_bytes=file_size, chunk_size_bytes=CHUNK_SIZE_BYTES,
            chunk_count=chunk_count, expires_at=timezone.now() + UPLOAD_TTL,
        )
        return {"upload": _session_payload(session)}
...
def receive_chunk(upload_id: str, chunk_index: int, payload: bytes, actor_email: str) -> dict[str, object]:
    ...
    with transaction.atomic():
        ...
        expected = (
            ...
            if index == int(session.chunk_count) - 1 else int(session.chunk_size_bytes)
        )
        if len(payload) != expected:
            raise _error("分片大小与预期不一致", status=422)
        digest = hashlib.sha256(payload).hexdigest()
        ErpReferenceRawUploadChunk.objects.update_or_create(
            session=session, chunk_index=index,
...
def _owner(session: ErpReferenceRawUploadSession, token: object) -> None:
...
def claim_upload(payload: dict[str, object], actor_email: str) -> dict[str, object]:
    if set(payload) != {"action", "uploadId", "source"} or payload.get("source") not in {"products", "combos"}:
        raise _error("上传接管字段集合无效")
    with transaction.atomic():
        lock_active_authority()
        session = _locked_session(payload["uploadId"], actor_email)
        if session.source_key != payload["source"]:
            raise _error("上传会话绑定的 ERP 来源不一致", code="version_conflict", status=409)
        ...
        if session.status == "processing" and session.updated_at > timezone.now() - PROCESSING_STALE_AGE:
            ...
            return {"kind": "claimed", "ownerToken": session.owner_token,
                    "upload": _session_payload(session), "chunks": list(
        ...
        session.expires_at = timezone.now() + UPLOAD_TTL
        session.save()
        return {"kind": "claimed", "ownerToken": owner_token, "upload": _session_payload(session),
                "chunks": list(session.chunks.order_by("chunk_index").values("chunk_index", "size_bytes", "sha256"))}
...
def finish_upload(payload: dict[str, object], actor_email: str) -> dict[str, object]:
    if set(payload) != {"action", "uploadId", "source", "ownerToken", "result"} or not isinstance(payload.get("result"), dict):
...
def release_upload(payload: dict[str, object], actor_email: str) -> dict[str, object]:
    if set(payload) != {"action", "uploadId", "source", "ownerToken"}:
