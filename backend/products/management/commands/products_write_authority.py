...
from .migrate_products_from_d1 import (
    GENERATION_VERSION,
    _counts,
    _sha,
    _source_snapshot,
...
def _source_authority(source: sqlite3.Connection) -> dict[str, object]:
    ...
    if row is None:
        raise CommandError("D1 商品经营 authority 尚未安装。")
    return dict(row)


def _assert_postgres_quiet() -> ProductWriteAuthority:
    authority = ProductWriteAuthority.objects.select_for_update().filter(id=1).first()
    control = ProductInventoryProjectionControl.objects.select_for_update().filter(id=1).first()
    if authority is None or control is None:
        raise CommandError("PostgreSQL 商品经营 authority 或库存投影控制行尚未初始化。")
    if (
        ProductWriteRequestReceipt.objects.filter(status="processing").exists()
        or ProductShippingRateImportBatch.objects.filter(status="processing").exists()
        or ProductImportAttempt.objects.filter(outcome="processing").exists()
        or ProductImportScopeHead.objects.exclude(status="ready").exists()
        or ProductImportScopeHead.objects.exclude(owner_token="").exists()
        or ProductRawUploadSession.objects.exclude(status="completed").exists()
        ...
        raise CommandError("PostgreSQL 商品经营写入状态不静默，拒绝 authority 变更。")
    return authority


def _verified_run(path: Path, snapshot: dict[str, object]) -> ProductMigrationRun:
    ...
    revision = ProductDataRevision.objects.filter(domain="products").first()
    if (
        run is None
        ...
        raise CommandError("当前 D1 快照没有一笔可激活的已复验商品经营迁移。")
    return run


class Command(BaseCommand):
    help = "Prepare, abort, activate, or inspect the products single-write authority."
    ...
    def handle(self, *args, **options) -> None:
        if (
            ...
            raise CommandError("生产商品经营 authority 只能由 migration_writer 进程角色操作。")
        ...
        try:
            ...
            target = ProductWriteAuthority.objects.filter(id=1).first()
            if not mutating:
                source.rollback()
            ...
            run = _verified_run(path, snapshot)
            if approved != run.id:
                raise CommandError("approved-run-id 与当前冻结商品经营事实不一致。")
            source_digest = _sha(snapshot)
            ...
            if prepare:
                with transaction.atomic():
                    ...
                    if target.status != "d1" or target.migration_verify_run_id != run.id:
                        raise CommandError("PostgreSQL 商品经营 authority 未绑定当前复验迁移。")
                if current["owner"] == "pending" and current["cutover_id"] == cutover_id:
                    source.rollback()
                elif current["owner"] == "d1":
                    changed = source.execute(
                        "UPDATE product_write_authority SET owner='pending',epoch=epoch+1,"
                        "cutover_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=1 AND owner='d1'",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 商品经营 authority prepare 未取得唯一所有权。")
                    source.commit()
                else:
                    raise CommandError("D1 商品经营 authority 无法进入本次 pending。")
                result = {"status": "prepared", "cutoverId": cutover_id, "approvedRunId": run.id, "sourceDigest": source_digest}
            elif abort:
                with transaction.atomic():
                    target = _assert_postgres_quiet()
                    if target.status != "d1":
                        raise CommandError("PostgreSQL 已取得商品经营写权，不能回退 pending。")
                if current["cutover_id"] != cutover_id:
                    raise CommandError("D1 不在本次商品经营 cutover 的 pending 状态。")
                if current["owner"] == "d1":
                    source.rollback()
                else:
                    changed = source.execute(
                        "UPDATE product_write_authority SET owner='d1',epoch=epoch+1,"
                        "updated_at=CURRENT_TIMESTAMP WHERE id=1 AND owner='pending' AND cutover_id=?",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 商品经营 authority pending 回退失败。")
                    source.commit()
                ...
                if current["cutover_id"] != cutover_id or current["owner"] not in {"pending", "postgresql"}:
                    raise CommandError("D1 必须先进入本次商品经营 cutover 的 pending 状态。")
                with transaction.atomic():
                    target = _assert_postgres_quiet()
                    if target.status == "d1":
                        ...
                        raise CommandError("PostgreSQL 商品经营 authority 与本次 cutover 冲突。")
                    epoch = str(target.authority_epoch)
                if current["owner"] == "pending":
                    changed = source.execute(
                        "UPDATE product_write_authority SET owner='postgresql',epoch=epoch+1,"
                        "updated_at=CURRENT_TIMESTAMP WHERE id=1 AND owner='pending' AND cutover_id=?",
                        (cutover_id,),
                    ).rowcount
                    if changed != 1:
                        raise CommandError("D1 商品经营 authority 终态写入失败；PostgreSQL 已安全持有写权。")
                    source.commit()
                else:
                    source.rollback()
            ...
            source.rollback()
            raise CommandError("D1 商品经营 authority 操作失败。") from error
        finally:
            source.close()
