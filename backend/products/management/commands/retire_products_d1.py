...
from .migrate_products_from_d1 import _counts, _sha, _source_snapshot
from .products_write_authority import (
    CUTOVER_ID_RE,
...
RETIREMENT_VERSION = "products-domain-retirement-receipt-v1"
SMOKE_VERSION = "products-system-test-receipt-v1"
HEX_64_RE = re.compile(r"^[0-9a-f]{64}$")
RUN_ID_RE = re.compile(r"^products-apply-[0-9a-f]{32}$")
MAX_RECEIPT_BYTES = 64 * 1024
REQUIRED_SMOKE_CHECKS = (
    "djangoReader",
    "djangoWriterNegative",
    "publicSummary",
    "publicShippingImport",
    "publicChunkUpload",
    "inventoryProjection",
    "aiConsumer",
    "globalSearchConsumer",
    "legacyD1Rejected",
    "inventoryD1Preserved",
)
TOMBSTONE_VIEWS = (
...
RETIREMENT_GUARDS = tuple(
    f"product_retired_{surface}_{operation}_guard"
    for surface in (
        "fingerprints",
        "attempts",
        "heads",
...
def _load_smoke_receipt(
    ...
    *,
    ...
    if (
        ...
        raise CommandError("系统测试 receipt 未证明完整商品经营垂直链路通过。")
...
def _migration_path() -> Path:
    path = (
        ...
        / "drizzle"
        / "0100_product_domain_retirement.sql"
    ).resolve()
    if not path.is_file():
        raise CommandError("缺少受控 D1 商品经营退役迁移。")
    return path


def _query_rows(source: sqlite3.Connection, query: str) -> list[dict[str, object]]:
...
def _shared_receipt(source: sqlite3.Connection) -> dict[str, object]:
    sections = {
        "fingerprints_other": _query_rows(
            ...
            "WHERE domain<>'product-shipping-rates' ORDER BY sequence",
        ),
        ...
        "heads_other": _query_rows(
            ...
            "WHERE domain<>'product-shipping-rates' ORDER BY domain,scope_key",
        ),
        "uploads_other": _query_rows(
            source,
            "SELECT * FROM inventory_import_uploads "
            "WHERE fingerprint NOT LIKE 'sku-shipping-rates:%' ORDER BY id",
        ),
        "upload_chunks_other": _query_rows(
            ...
            "WHERE u.fingerprint NOT LIKE 'sku-shipping-rates:%' "
        ...
        "upload_results_other": _query_rows(
            ...
            "WHERE u.fingerprint NOT LIKE 'sku-shipping-rates:%' ORDER BY r.upload_id",
        ),
        "retirement_receipts_other": _query_rows(
            source,
            "SELECT * FROM domain_retirement_receipts "
            "WHERE domain<>'products' ORDER BY domain",
        ) if source.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' "
            "AND name='domain_retirement_receipts'"
        ).fetchone() else [],
    }
...
def _product_upload_chunk_count(source: sqlite3.Connection) -> int:
    return int(
        source.execute(
...
def _build_plan(
    ...
    *,
    ...
    d1_authority = source.execute(
        "SELECT owner,epoch,cutover_id FROM product_write_authority WHERE id=1"
    ).fetchone()
    if (
        ...
        raise CommandError("D1 商品经营 authority 不属于本次已激活 cutover。")
    product_chunks = _product_upload_chunk_count(source)
    if product_chunks:
        raise CommandError("D1 商品经营旧分片对象键尚未在切换前受控清理。")
    with transaction.atomic():
        target = _assert_postgres_quiet()
        run = _verified_run(source_path, snapshot)
        if run.id != approved_run_id:
            raise CommandError("approved-run-id 与终态 D1 商品经营快照不一致。")
        if (
            ...
            raise CommandError("PostgreSQL 商品经营 authority 不属于本次完成迁移。")
    ...
    preflight = {
        ...
        "migrationRunId": run.id,
        "sourceDigest": source_digest,
        "productUploadChunks": product_chunks,
        "shared": shared,
    }
...
def _completed_receipt(source: sqlite3.Connection) -> dict[str, object] | None:
    ...
    row = source.execute(
        ...
        "FROM domain_retirement_receipts WHERE domain='products'"
    ).fetchone()
...
def _verify_retired(source: sqlite3.Connection, receipt: dict[str, object]) -> None:
    if (
        ...
        raise CommandError("D1 商品经营退役 receipt 无效。")
    views = {
    ...
    if set(views) != set(TOMBSTONE_VIEWS) or any(
        "products-domain-retired-v1" not in sql for sql in views.values()
    ):
        raise CommandError("D1 商品经营退役 tombstone view 集合不完整。")
    for view in TOMBSTONE_VIEWS:
        if int(source.execute(f'SELECT COUNT(*) FROM "{view}"').fetchone()[0]) != 0:
            raise CommandError("D1 商品经营退役 tombstone view 非空。")
    guards = {
        str(row[0])
        for row in source.execute(
            "SELECT name FROM sqlite_master WHERE type='trigger' "
            "AND name LIKE 'product_retired_%_guard'"
        )
    }
    if guards != set(RETIREMENT_GUARDS):
        raise CommandError("D1 商品经营退役共享表 guard 集合不完整。")
    for table in (
        ...
        if int(
            ...
            raise CommandError("D1 共享导入表仍含商品经营域行。")
    if int(
        source.execute(
            "SELECT COUNT(*) FROM inventory_import_uploads "
            "WHERE fingerprint LIKE 'sku-shipping-rates:%'"
        ).fetchone()[0]
    ):
        raise CommandError("D1 共享上传表仍含商品经营域行。")
...
def _statements(payload: bytes) -> list[str]:
    try:
        ...
        raise CommandError("D1 商品经营退役迁移不是 UTF-8。") from error
    statements = [
        item.strip()
        for item in text.split("--> statement-breakpoint")
        if item.strip()
    ]
    if (
        len(statements) < 30
        or not statements[0].startswith("-- Operator-only terminal retirement")
    ):
        raise CommandError("D1 商品经营退役迁移结构不符合契约。")
    return statements
...
class Command(BaseCommand):
    help = "Plan or execute terminal D1 retirement after the products PostgreSQL cutover."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        if (
            ...
            raise CommandError("生产 D1 商品经营退役只能由 migration_writer 进程角色操作。")
        ...
        if not source_input.is_file() or source_input.is_symlink():
            raise CommandError("D1 商品经营源必须是普通文件。")
        ...
        try:
            ...
            if completed is not None:
                ...
                if (
                    ...
                    or completed["plan_id"]
                    != (approved_plan_id or completed["plan_id"])
                ):
                    raise CommandError("既有 D1 商品经营退役 receipt 与本次请求冲突。")
                source.rollback()
                result = {
            ...
            if receipt is None:
                raise CommandError("D1 商品经营退役未生成完成 receipt。")
            _verify_retired(source, receipt)
            if _shared_receipt(source) != plan["shared"]:
                raise CommandError("D1 商品经营退役改变了其他域共享状态。")
            source.commit()
            result = {
                "status": "retired",
                "version": RETIREMENT_VERSION,
            ...
            source.rollback()
            raise CommandError("D1 商品经营退役事务失败。") from error
        finally:
            source.close()
