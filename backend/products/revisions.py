...
from .models import ProductDataRevision


def revision_value(*, for_update: bool = False) -> str:
    query = ProductDataRevision.objects
    if for_update:
        query = query.select_for_update()
    row = query.get(domain="products")
...
def bump_revision(reason: object) -> str:
    with transaction.atomic():
        row = ProductDataRevision.objects.select_for_update().get(domain="products")
        payload = json.dumps(
