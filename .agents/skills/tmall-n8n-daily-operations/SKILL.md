...
- **状态确认**：只读核验所有 `enabled=true` 店铺的 live workflow、当日 execution、helper、活动清单、精确导入批次和日期覆盖；不触发业务动作。
...
1. 从当前注册表与工作流定义生成应监控集合，核验每个 `enabled=true` 店铺的仓库模板与 n8n live `nodes/connections/settings` 一致；每店只能有一个 active 目标日调度，未启用店铺不得有 active 调度，所有天猫流程共用同一 helper 协调门禁。时区、cron、店铺键和节点顺序必须正确，不能把亿玖店的固定 ID 或 `0 11 * * *` 套到其他店铺。
2. 按每店自己的计划时间读取当日 `mode=trigger` execution。运行中或在 A 前等待 helper 都持续监控到终态；计划时间后 5 分钟仍不存在时检查同一 workflow ID 的 trigger 注册和服务状态，不能创建并发副本。一次值班要覆盖全部已启用店铺，单店失败或等待不得导致其余店铺漏检。
