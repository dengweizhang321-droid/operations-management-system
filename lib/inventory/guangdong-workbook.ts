...
export function monitorWorkbook(data: GuangdongMonitor): Uint8Array {
  return workbookBytes([
    { name: "广东入仓监控", rows: [
      ["货品编码", "货品名称", "规格编码", "规格", "品牌", "品类", "供应商", "供应商来源", "仓库", "可用库存", "在途", "7日出库", "15日出库", "30日出库", "销售周转天数", "库龄天数", "生产周期天", "安全天数", "最晚下单日期", "备货数量", "最新下单日期", "风险", "风险原因", "备注"],
      ...data.items.map((r) => [r.productCode, r.productName, r.productCode, r.specification, r.brand, r.category, r.supplier, r.supplierSource, r.warehouse, r.availableQuantity, r.inTransitQuantity, r.outbound7dQuantity, r.outbound15dQuantity, r.outbound30dQuantity, r.turnoverDays, r.inventoryAgeDays, r.leadDays, r.bufferDays, r.latestOrderDate, r.replenishmentQuantity, r.latestReplenishmentOrderDate, r.riskLabel, r.riskReasons.join("；"), r.notes]),
    ] },
