...
- 市场图片交互式云端识别默认保持双通道，但每个 HTTP 请求只处理一张图片；同一任务服务端最多允许两个有效推理租约。视觉模型调用、浏览器请求与推理租约依次限定为最多 90 秒、110 秒和 3 分钟。模型超时、网络异常或供应商限流时，客户端必须自动刷新任务状态、降为单通道并以 15 秒至 5 分钟有界指数退避续跑；连接连续成功 3 张后自动恢复双通道。续跑必须复用兼容的原任务，不得因客户端超时重复创建任务或覆盖已完成结果。
