...
- 天猫货品、生意参谋商品日和推广导入固定绑定 `platform=天猫`，`shop_name` 必须来自 `config/tmall-store-accounts.json` 中已启用的受控店铺；未提供店铺名时仅允许注册表恰有一个启用店铺的兼容路径，不得接受未注册或未启用店铺。每店使用独立 profile、调试端口、下载目录、签收单和恢复清单并串行运行，切店后必须核验页面可见店铺身份。商品日与推广日按预期业务日期完整校验；推广费率和推广成交占比只按两类报表的日期交集计算，商品访客只表达商品×日累计，不得估算为店铺去重 UV。阿里妈妈全站推自动化只能在商品日签收导入成功后运行，按运营系统推广覆盖选择最早连续缺口，强制选择“全部数据指标”；平台消息或广告只允许点击明确的关闭、忽略、暂不或稍后动作。确认生成报表后，只能点击同时包含“离线数据生成成功”和“下载任务管理”语境的成功提示内“立即前往/前往下载”，不得把广告或普通前往按钮当作下载入口；恢复已提交任务时才允许使用受控下载任务地址。下载中心必须按目标日期、任务创建时间、成功状态和唯一下载动作锁定本轮任务，不得选择历史任务；ZIP 只有在单 CSV、商品主体、全日期、店铺、大小、行数和哈希均通过并完成落库覆盖回查后才算成功。
