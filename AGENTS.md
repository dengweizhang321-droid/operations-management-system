...
- “工作流”板块中的 n8n 编辑器只允许 `operator`、`admin` 加载，`viewer`、`analyst` 只能查看原生流程概览。嵌入页必须继续依赖 n8n 自身登录态，不得把 n8n Cookie、账号、密码或 Token 代理进运营系统，也不得在页面加载时自动发布、激活或执行工作流；工作流默认保持 `active=false`，真实运行仍需显式启动受控的本机环回辅助进程。
