...
- 市场 SKU 图片交互式 AI 标注的模型并发数按“三级类目 + 执行器”持久化，范围为 1–50；云端默认 10、建议 10–20，本地 Ollama 默认且建议 1。每个 HTTP 请求只处理一张图片，服务端必须使用同一持久化配置和原子条件限制任务的有效推理租约，不能只靠前端 worker 数防并发越界。视觉模型调用、浏览器请求与推理租约依次限定为最多 90 秒、110 秒和 3 分钟。模型超时、网络异常或供应商限流时，客户端必须自动刷新任务状态、临时降为并发 1，并以 15 秒至 5 分钟有界指数退避续跑；连接连续成功 3 张后自动恢复该类目的配置值。续跑必须复用兼容的原任务，不得因客户端超时重复创建任务或覆盖已完成结果。
