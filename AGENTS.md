...
- 天猫货品、生意参谋商品日和推广导入固定绑定 `platform=天猫`，`shop_name` 必须来自 `config/tmall-store-accounts.json` 中已启用的受控店铺；未提供店铺名时仅允许注册表恰有一个启用店铺的兼容路径，不得接受未注册或未启用店铺。每店使用独立 profile、调试端口、下载目录、签收单和恢复清单并串行运行，切店后必须核验页面可见店铺身份。商品日与推广日按预期业务日期完整校验；推广费率和推广成交占比只按两类报表的日期交集计算，商品访客只表达商品×日累计，不得估算为店铺去重 UV。阿里妈妈全站推自动化只能在商品日签收导入成功后运行，且入口固定为千牛店铺后台最左侧“推广”→推广中心左侧“货品全站推”→全站推页面左侧“报表”→“下载报表”，不得从阿里妈妈首页直接寻找推广入口；推广缺口必须按日期升序逐日串行处理，每个报表的起止日期必须为同一个业务日并强制选择“全部数据指标”，前一天完成下载、文件校验、导入和同日覆盖回查后才可进入下一天，任意一天失败即停止，已覆盖日期重跑时跳过。平台消息或广告只允许点击明确的关闭、忽略、暂不或稍后动作。确认生成报表后，只能点击同时包含“离线数据生成成功”和“下载任务管理”语境的成功提示内“立即前往/前往下载”，不得把广告或普通前往按钮当作下载入口；恢复已提交任务时才允许使用受控下载任务地址。下载中心必须按唯一目标日、任务创建时间、成功状态和唯一下载动作锁定本轮任务，不得选择历史任务；ZIP 只有在单 CSV、商品主体、唯一目标日、店铺、大小、行数和哈希均通过并完成落库覆盖回查后才算成功。
- “工作流”板块中的 n8n 编辑器只允许 `operator`、`admin` 加载，`viewer`、`analyst` 只能查看原生流程概览；即使角色允许，helper 处于 checking、offline、busy 或所需 profile/Cookie 缺失时也不得挂载 iframe 或 n8n 外链。嵌入页必须继续依赖 n8n 自身登录态，不得把 n8n Cookie、账号、密码或 Token 代理进运营系统，也不得在页面加载时自动发布、激活或执行工作流；工作流默认保持 `active=false`。本地 Worker 启动器必须守护只监听 `127.0.0.1:5791` 的一次性辅助进程并在每轮退出后有界退避重启；同一多段工作流必须以 n8n execution ID 绑定一次性 helper 的唯一执行所有者，空 ID、跨执行接管、并发或乱序请求失败关闭。页面健康检查只能返回阶段、忙碌状态和 Cookie/Profile 原文件是否存在，不得返回路径或内容。缺口规划和导入幂等仍由后端按店铺、数据集、日期和文件内容判定，不得改成前端去重。
