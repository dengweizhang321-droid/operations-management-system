...
- “工作流”板块中的 n8n 编辑器只允许 `operator`、`admin` 加载，`viewer`、`analyst` 只能查看原生流程概览。嵌入页必须继续依赖 n8n 自身登录态，不得把 n8n Cookie、账号、密码或 Token 代理进运营系统，也不得在页面加载时自动发布、激活或执行工作流；工作流默认保持 `active=false`。本地 Worker 启动器必须守护只监听 `127.0.0.1:5791` 的一次性辅助进程并在每轮退出后有界退避重启；页面健康检查只能返回阶段、忙碌状态和 Cookie 原文件是否存在，不得返回路径或内容，离线或文件缺失时必须阻止嵌入画布执行。缺口规划和导入幂等仍由后端按店铺、数据集、日期和文件内容判定，不得改成前端去重。
