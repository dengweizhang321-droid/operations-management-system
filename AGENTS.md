...
- 市场 SKU 图片交互式 AI 标注的模型并发数按“三级类目 + 执行器”持久化，范围为 1–50；云端默认 10、建议 10–20，本地 Ollama 默认且建议 1。新任务设置与当前任务控制必须分区展示，当前任务运行中保存并发后要同时更新服务端租约上限和当前浏览器 worker 目标。每个 HTTP 请求只处理一张图片，服务端必须使用同一持久化配置和原子条件限制任务的有效推理租约，不能只靠前端 worker 数防并发越界。视觉模型调用、浏览器请求与推理租约依次限定为最多 90 秒、110 秒和 3 分钟。单次模型超时或网络异常先把当前运行并发降到约 75%（配置不少于 4 时首次不低于 2），从 5 秒短冷却开始，重复异常再阶梯减半且普通异常冷却最多 30 秒；同一冷却窗口内并发返回的同类失败只计为一次退避事件。供应商限流按当前并发减半并使用 60 秒至 5 分钟有界退避或供应商 `Retry-After`；租约占满只让对应 worker 短等，不得暂停其他通道。连接每连续成功 3 张增加一路，直至恢复该类目的配置值。续跑必须复用兼容的原任务，不得因客户端超时重复创建任务或覆盖已完成结果。
