"""Isolated 0056 -> 0057 closed budget attestation upgrade and restore."""
import argparse
import hashlib
import importlib
import json
...
from ai_assistant.table_manifest import (
    AI_TABLES_PRE_BUDGET_V10_ATTESTATIONS as OLD_TABLES, AI_TABLES)
...
seed = folder / "business-market-v2-role-bridge-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0055->0056"):
    raise RuntimeError("0057 requires verified isolated 0056 predecessor")
if len(OLD_TABLES) != 80 or len(AI_TABLES) != 81:
    raise AssertionError("0057 expected frozen80/live81 AI table inventory")

OLD = [("ai_assistant", "0056_business_market_v2_material_role_bridge"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0057_business_promotion_budget_v10_attestation"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0056_business_market_v2_material_role_bridge',"
        "'0057_business_promotion_budget_v10_attestation') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0057 requires exact 0056 predecessor without 0057")
...
NEW_FUNCTIONS = (migration.GUARD, migration.ATTEST)
NEW_TRIGGERS = {"ai_budget_v10_attestation_guard",
...
def archive_restore(name):
    ...
    for executable, arguments in (
        ...
        if done.returncode:
            ...
            raise RuntimeError("isolated 0057 archive/restore failed")
...
def ready_guards(db):
    ...
    expected = (closed.RUN_GUARD, closed.COMPLETE_GUARD,
                closed.previous.READY_REQUIREMENTS)
    for signature, definition in zip(names, expected):
        row = db.execute("SELECT p.prosrc FROM pg_catalog.pg_proc p "
    ...
    if ("ai_promotion_budget_renderer_unpublished" not in closed.RUN_GUARD
            or "ai_promotion_budget_renderer_unpublished" not in closed.COMPLETE_GUARD):
        raise AssertionError("v10 ready is not default closed")
    try:
...
def attestation_closed(db, *, installed):
    ...
    helper = db.execute("SELECT to_regprocedure(%s)::text", [signature]).fetchone()[0]
    relation = db.execute("SELECT to_regclass(%s)::text", [migration.TABLE]).fetchone()[0]
    if not installed:
        if helper is not None or relation is not None:
            raise AssertionError("0057 function or table present before install")
        if role is not None and role != (False,) * 7:
            raise AssertionError("retained attestor role was activated")
        return
    if role != (False,) * 7 or helper is None or relation is None:
        raise AssertionError("0057 role, function or table missing")
    if db.execute("SELECT count(*) FROM pg_catalog.pg_auth_members WHERE "
        ...
        raise AssertionError("0057 attestor role gained membership")
    if db.execute("SELECT count(*) FROM " + migration.TABLE).fetchone() != (0,):
        raise AssertionError("0057 empty inverse requires no attestation")
    with db.cursor() as cursor:
        migration.verify_catalog(cursor)


with connect() as db:
    attestation_closed(db, installed=False)
    ready_guards(db)
    before = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
archive_restore("budget_v10_attestation_before")
with connect("budget_v10_attestation_before") as copy:
    attestation_closed(copy, installed=False)
    ready_guards(copy)
    ...
    if (restored[0], restored[1], normalized(restored[2]),
        ...
        raise AssertionError("0056 pre-upgrade independent restore differs")

MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    attestation_closed(db, installed=True)
    ready_guards(db)
    after = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
    if after[:2] != before[:2]:
        raise AssertionError("0057 changed 80 historical tables or renderer bytes")
    for name, old in before[2].items():
        if after[2].get(name) != old:
            raise AssertionError("0057 changed old AI function OID/body/ACL: " + name)
    new_signatures = {db.execute("SELECT to_regprocedure(%s)::text",
        [signature]).fetchone()[0] for signature in (
        "public.ai_budget_v10_attestation_guard()", migration.SIGNATURE)}
    if None in new_signatures or len(new_signatures) != 2 or (
            set(after[2]) != set(before[2]) | new_signatures):
        raise AssertionError("0057 did not add exactly two AI functions")
    for signature, definition in (
            ("public.ai_budget_v10_attestation_guard()", migration.GUARD),
            (migration.SIGNATURE, migration.ATTEST)):
        key = db.execute("SELECT to_regprocedure(%s)::text",
            [signature]).fetchone()[0]
        if after[2][key][1] != definition.split("$$", 2)[1]:
            raise AssertionError("0057 new function body drift")
    old_rel, old_trg = before[3]
    new_rel, new_trg = after[3]
    old_rel_names = {row[1] for row in old_rel}
    old_trg_names = {(row[1], row[2]) for row in old_trg}
    if any(row not in new_rel for row in old_rel) or any(
            row not in new_trg for row in old_trg):
        raise AssertionError("0057 changed old relation or trigger identity")
    added_relations = [row for row in new_rel if row[1] not in old_rel_names]
    added_triggers = [row for row in new_trg if (row[1], row[2]) not in old_trg_names]
    if (len(added_relations) != 4
            or {row[2] for row in added_relations} != {"r", "i"}
            or {row[1] for row in added_relations if row[2] == "r"}
                != {migration.TABLE.removeprefix("public.")}
            or {row[2] for row in added_triggers} != {
                "ai_budget_v10_attestation_guard",
                "ai_budget_v10_attestation_no_truncate"}):
        raise AssertionError("0057 relation/index/trigger footprint is not exact")
archive_restore("budget_v10_attestation_after")
with connect("budget_v10_attestation_after") as copy:
    attestation_closed(copy, installed=True)
    ready_guards(copy)
    restored = (old_table_digest(copy), old_files(copy), function_catalog(copy),
        relation_trigger_catalog(copy))
    if (restored[0], restored[1], normalized(restored[2]),
        ...
        raise AssertionError("0057 post-upgrade independent restore differs")
...
with connect() as db:
    ...
    reverted = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
    if reverted != before:
        raise AssertionError("0057 empty reverse did not restore exact 0056 state")
MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    attestation_closed(db, installed=True)
    ready_guards(db)
    ...
    if (replayed[0], replayed[1], normalized(replayed[2]),
            restored_inventory(replayed[3])) != (
            after[0], after[1], normalized(after[2]),
            restored_inventory(after[3])):
        raise AssertionError("0057 reapply differed from first installation")

result = {"upgrade": "0056->0057", "oldAiTables": 80,
    "newAiTables": 81, "oldRowsDigestPreserved": before[0],
    "rendererVersions": list(before[1]), "rendererBytesPreserved": True,
    "oldAiFunctionOidBodyAclPreserved": True,
    "v9ReadyDenialAndV10ReadyClosed": True,
    "newVerifierNoLoginNoMembership": True,
    "newAttestationFunctionAclBodyExact": True,
    "newTableAndTriggersClosed": True,
    "beforeBackupRestored": True, "afterBackupRestored": True,
    "emptyReverseAndReapply": True, "productionWrites": False}
(folder / "business-promotion-budget-v10-attestation-upgrade-evidence.json").write_text(
