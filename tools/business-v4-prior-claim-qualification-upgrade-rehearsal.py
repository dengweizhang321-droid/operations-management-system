"""Isolated 0050 -> 0051 prior-claim writer fix, restore, and empty reversal."""
import argparse
import hashlib
import importlib
import json
...
seed = folder / "business-v4-replay-read-cast-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0049->0050"):
    raise RuntimeError("0051 rehearsal requires verified isolated 0050 seed")

OLD = [("ai_assistant", "0050_business_v4_replay_read_cast"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0051_business_v4_prior_claim_column"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0050_business_v4_replay_read_cast',"
        "'0051_business_v4_prior_claim_column') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0051 requires exact 0050 predecessor without 0051")
...
record_migration = importlib.import_module(
    "ai_assistant.migrations.0047_business_v4_sealer_replay_progress")
RECORD = record_migration.WRITE
OLD_BODY = old_migration.RECORD.split("$$")[1]
NEW_BODY = new_migration.RECORD.split("$$")[1]
if OLD_BODY == NEW_BODY:
    raise RuntimeError("0051 rehearsal requires a distinct prior-claim writer")
...
def record_key(db):
    key = db.execute("SELECT to_regprocedure(%s)::text", [RECORD]).fetchone()[0]
    if key is None:
        raise AssertionError("replay writer missing")
    return key
    ...
    for signature, old in before.items():
        ...
        if signature == key:
            if (old[0] != now[0] or old[2:] != now[2:]
                ...
                raise AssertionError("RECORD OID, ACL, signature, or non-body property drift")
        elif now != old:
            raise AssertionError("non-RECORD AI function changed: " + signature)
...
def closed(db, *, qualified=False):
    ...
    body = db.execute("SELECT prosrc FROM pg_catalog.pg_proc "
        "WHERE oid=to_regprocedure(%s)", [RECORD]).fetchone()
    if body != (NEW_BODY if qualified else OLD_BODY,):
        raise AssertionError("replay writer body mismatch")
...
with connect() as db:
    ...
    writer = record_key(db)

archive_restore("v4_prior_claim_qualification_before")
with connect("v4_prior_claim_qualification_before") as restored:
    closed(restored)
...
with connect() as db:
    closed(db, qualified=True)
    after = (old_table_digest(db), old_files(db), function_catalog(db))
    assert after[:2] == before[:2]
    assert_catalog_change(before[2], after[2], writer, NEW_BODY)

archive_restore("v4_prior_claim_qualification_after")
with connect("v4_prior_claim_qualification_after") as restored:
    closed(restored, qualified=True)
    assert (old_table_digest(restored), old_files(restored),
        restorable_catalog(function_catalog(restored))) == (
...
result = {"upgrade": "0050->0051", "aiTables": 79,
    "oldRowsAndRendererBytesPreserved": True,
    "onlyRecordBodyChanged": True,
    "recordOidAclAndSignaturePreserved": True,
...
(folder / "business-v4-prior-claim-qualification-upgrade-evidence.json").write_text(
