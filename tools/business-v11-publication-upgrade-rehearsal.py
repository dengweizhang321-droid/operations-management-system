"""Disposable 0077→0078 publication upgrade, frozen predecessor and dual empty restore."""
...
OLD = ("ai_assistant", "0077_business_market_v6_paused_topology")
NEW = ("ai_assistant", "0078_business_promotion_budget_v11_signed_publication")
candidate = importlib.import_module("ai_assistant.migrations." + NEW[1])
old77 = importlib.import_module("ai_assistant.migrations." + OLD[1])
...
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
    ...
    raise RuntimeError("0078 upgrade requires exact disposable PostgreSQL")
...
def native(command, timeout=600):
    ...
    if result.returncode:
        raise RuntimeError("0078 archive command failed; diagnosticSha256=" +
...
def assert_empty(source, expected):
    if protected_names(source) != expected:
        raise RuntimeError("0078 protected table inventory drift")
    for table in expected:
        row = source.execute(sql.SQL("SELECT EXISTS(SELECT 1 FROM {} LIMIT 1)")
            .format(sql.Identifier("public", table))).fetchone()
        if row != (False,):
            raise RuntimeError("0078 refuses clear archive of protected data")
...
def function_catalog(source):
    with source.cursor() as cursor:
        ...
        signatures += [item[0] for item in candidate._frozen(cursor)]
    ...
    for signature in signatures:
        row = source.execute("SELECT oid,prosrc,proacl::text,"
            ...
            [signature]).fetchone()
        if row is None:
            raise RuntimeError("0078 predecessor function missing")
...
def table_catalog(source, names):
    ...
    for name in sorted(names):
        ...
        if row is None:
            raise RuntimeError("0078 predecessor protected table missing")
...
def archive_restore(name, expected):
    ...
    if path.exists():
        raise RuntimeError("0078 isolated archive target exists")
    with db() as source:
...
def assert_formal_backup_closed():
    ...
    target = folder / "v11_publication_unadmitted_formal.dump"
    if target.exists():
        raise RuntimeError("0078 formal backup target preexists")
    ...
    try:
        ...
        raise AssertionError("0078 formal refusal unstructured") from None
    ...
    if (result.returncode != 1 or target.exists() or
        ...
        raise AssertionError("0078 formal backup did not reject before archive")
...
def assert_formal_restore_closed(archive):
    env={**os.environ,"PGHOST":str(database["HOST"]),
        "PGPORT":str(database["PORT"]),"PGUSER":str(database["USER"]),
        "PGPASSWORD":str(database["PASSWORD"])}
    target="v11_publication_formal_restore_forbidden"
    with db() as source:
        if source.execute("SELECT 1 FROM pg_catalog.pg_database WHERE datname=%s",
                [target]).fetchone() is not None:
            raise RuntimeError("0078 formal restore target already exists")
    ...
    try:
        ...
        raise AssertionError("0078 formal restore refusal unstructured") from None
    ...
    if (result.returncode!=1 or not target_absent or
            refused.get("status")!="failed" or
            refused.get("errorSha256")!=expected):
        raise AssertionError("0078 formal restore did not reject before DB")
...
def assert_formal_migration_closed():
    ...
    if (result.returncode!=0 or b"formal blocked" not in result.stdout):
        raise AssertionError("0078 formal PrepareApp/DeployApp gate not proven")
...
with db() as source:
    ...
    if receipts != [(OLD[1],)]:
        raise RuntimeError("0078 requires exact 0077 predecessor")
    with source.cursor() as cursor:
        old77.verify_catalog(cursor)
...
before_dump = archive_restore("v11_publication_before", OLD_TABLES)
with db("v11_publication_before") as restored:
    if (not restored_equal(function_catalog(restored), before_functions) or
        ...
        raise AssertionError("0078 before independent restore drift")
...
if [(migration.app_label,migration.name,backward) for migration,backward
    ...
    raise RuntimeError("0078 Django forward plan is not exact")
...
with db() as source:
    ...
    if (function_catalog(source) != before_functions or
        ...
        raise AssertionError("0078 changed predecessor OID/body/ACL/owner")
assert_formal_backup_closed()
after_dump = archive_restore("v11_publication_after", NEW_TABLES)
assert_formal_restore_closed(after_dump)
assert_formal_migration_closed()
with db("v11_publication_after") as restored:
    with restored.cursor() as cursor:
        candidate.verify_catalog(cursor)
    if (not restored_equal(function_catalog(restored), before_functions) or
            table_catalog(restored, OLD_TABLES) != before_tables or
            legacy_file_rows(restored) != before_files):
        raise AssertionError("0078 after independent restore drift")

MigrationExecutor(connection).migrate([OLD])
with db() as source:
    with source.cursor() as cursor:
        candidate._role(cursor,candidate.v2.READ_ROLE)
        cursor.execute("SELECT to_regclass(%s)",[candidate.TABLE])
        if cursor.fetchone()!=(None,):
            raise AssertionError("0078 empty reverse retained publication table")
        for signature in candidate.v2.SIGNATURES:
            cursor.execute("SELECT to_regprocedure(%s)",[signature])
            if cursor.fetchone()!=(None,):
                raise AssertionError("0078 empty reverse retained function")
    if (function_catalog(source) != before_functions or
            table_catalog(source, OLD_TABLES) != before_tables or
            legacy_file_rows(source) != before_files or
            protected_names(source) != OLD_TABLES):
        raise AssertionError("0078 empty reverse changed predecessor")
MigrationExecutor(connection).migrate([NEW])
with db() as source:
    with source.cursor() as cursor:
        candidate.verify_catalog(cursor)
    if function_catalog(source) != before_functions:
        raise AssertionError("0078 reapply changed predecessor functions")

result = {"upgrade": "0077->0078", "status": "passed",
    ...
    "newFunctionCount": len(candidate.v2.SIGNATURES),
    ...
    "emptyReversePreservedNoLoginReaderRole": True,
...
(folder / "business-v11-publication-upgrade-evidence.json").write_text(
