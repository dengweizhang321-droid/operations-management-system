...
const root=process.cwd(), directory=path.resolve(process.argv[2]||".runtime/business-scope-picker-ui");
...
import Picker from ${JSON.stringify(path.join(root,"app/ai-business-scope-picker.tsx"))};
...
function fixture(query,cursor){
  ...
  let items=Array.from({length:count},(_,i)=>{const identity={platform:"京东",shop:`合成店${String(i).padStart(4,"0")}`,dataset:"sku"};return {optionKey:sha({domain:"netshop",identity}),identity,source:"jd_sku_daily",sourceDataset:"sku_daily",
  ...
  items=items.filter(item=>Object.entries(query).every(([k,v])=>k==="q"?item.identity.shop.includes(v)||item.identity.platform.includes(v):item.identity[k]===v));
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname.startsWith("/api/")){
      requests.push({method:req.method,path:url.pathname,query:url.search});assert.equal(req.method,"GET");assert.equal(url.pathname,"/api/ai/business-plan/netshop-options");
      ...
      const value={schemaVersion:"business-netshop-options-response-v1",principalKey:mode==="wrong-key"?keyB:actor,page};
...
const picker=()=>page.getByRole("region",{name:"选择经营分析来源",exact:true});
const add=()=>picker().getByRole("button",{name:/^添加来源 /});
const events=async()=>JSON.parse(await page.locator("#events").innerText());
const ready=async(expected=20)=>{await page.waitForFunction(n=>[...document.querySelectorAll('button[aria-label^="添加来源 "]')].length===n,expected);await page.getByRole("button",{name:"取消来源读取",exact:true}).waitFor({state:"detached"});};
...
try{
  ...
  await check("manual_selection_revalidates_identity_and_never_changes_dates_or_submits",async()=>{
    ...
    assert.equal(requests.length,before+1);assert.equal((await events())[0].identity.shop,"合成店0000");assert.equal((await events())[0].identity.dataset,"sku");
  ...
  await check("twenty_then_three_with_previous_and_exact_filters",async()=>{
    await picker().getByRole("button",{name:"下一页来源",exact:true}).click();await ready(3);assert.match(await add().first().getAttribute("aria-label"),/0020/);
    await picker().getByRole("button",{name:"上一页来源",exact:true}).click();await ready();assert.match(await add().first().getAttribute("aria-label"),/0000/);
    await picker().getByLabel("来源平台",{exact:true}).selectOption("京东");await picker().getByLabel("来源精确店铺",{exact:true}).fill("合成店0022");await picker().getByLabel("来源数据集",{exact:true}).selectOption("sku");await read();await ready(1);
    assert.match(requests.at(-1).query,/dataset=sku/);assert.match(await add().first().getAttribute("aria-label"),/0022/);
    await picker().getByLabel("来源名称搜索",{exact:true}).fill("无此名称");await read();await ready(0);await picker().getByText("当前筛选没有已发布来源；这不等于没有经营活动。",{exact:true}).waitFor();
  });
  ...
  await check("revision_409_requires_explicit_first_page_reread",async()=>{
    mode="conflict";await picker().getByRole("button",{name:"下一页来源",exact:true}).click();await picker().getByRole("alert").filter({hasText:"从首页重读"}).waitFor();assert.equal(await add().count(),0);
  ...
  await check("late_old_filter_response_cannot_replace_new_page",async()=>{
    await reload();delay=true;started=false;await read();await waitDelay();await picker().getByLabel("来源名称搜索",{exact:true}).fill("合成店0022");await read();await ready(1);await waitFinished();assert.equal(await add().count(),1);assert.match(await add().first().getAttribute("aria-label"),/0022/);
  });
  await check("cancel_pending_selection_never_emits",async()=>{
    await reload();delay=true;started=false;await add().first().click();await waitDelay();await picker().getByRole("button",{name:"取消来源读取",exact:true}).click();await waitFinished();assert.equal((await events()).length,0);assert.equal(await add().count(),0);
  });
  ...
  await check("hundred_page_bound_reports_remaining_instead_of_clipping",async()=>{
    count=2001;await read();await ready();for(let n=2;n<=100;n++){await picker().getByRole("button",{name:"下一页来源",exact:true}).click();await picker().getByText(`第 ${n} 页 · 本页 20 个来源 · 后面还有来源`,{exact:true}).waitFor();}
    assert.equal(await picker().getByRole("button",{name:"下一页来源",exact:true}).isDisabled(),true);await picker().getByText("已浏览 100 页，仍有来源未展示。请缩小筛选后从首页重读。",{exact:true}).waitFor();assert.equal(await add().count(),20);
  });
