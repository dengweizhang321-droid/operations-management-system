"""Isolated 0061 -> 0062 dormant market read receipt upgrade/restore."""
import argparse
import hashlib
import importlib
import json
...
from ai_assistant.table_manifest import (
    AI_TABLES_PRE_MARKET_V2_READ_RECEIPTS as OLD_AI_TABLES,
    AI_TABLES_PRE_MARKET_V2_EXECUTION_PLANS as AI_TABLES)
...
seed = folder / "business-market-v2-context-proof-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0060->0061"
        or len(OLD_AI_TABLES) != 82 or len(AI_TABLES) != 83):
    raise RuntimeError("0062 requires verified isolated 0061 82-table predecessor")

OLD = [("ai_assistant", "0061_business_market_v2_context_proof"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0062_business_market_v2_read_receipt_candidate"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0061_business_market_v2_context_proof',"
        "'0062_business_market_v2_read_receipt_candidate') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0062 requires exact 0061 predecessor without 0062")
...
GUARDS = {
    "public.ai_market_v2_read_receipt_guard()": (migration.GUARD, False),
...
TRIGGERS = {
    ("ai_business_market_v2_read_receipts", "ai_market_v2_read_receipt_guard"):
        "public.ai_market_v2_read_receipt_guard()",
    ("ai_business_market_v2_read_receipts", "ai_market_v2_read_receipt_no_truncate"):
...
def market_roots(db):
    ...
    return reports, flows, materials, proofs
...
def archive_restore(name):
    ...
    for executable, arguments in (
        ...
        if done.returncode:
            ...
            raise RuntimeError("isolated 0062 archive/restore failed")
...
def closed(db, installed):
    ...
    if new_rows != (0, 0):
        raise AssertionError("empty 0062 upgrade seed gained execution rows")
...
def verify_install(db, before, after):
    if after[:3] != before[:3]:
        raise AssertionError("0062 changed old 82 rows, market roots, proofs or renderers")
    ...
    if (set(new_relations) != set(old_relations) | {
            ...
            "ai_business_market_v2_read_receipts_pkey"}
            or any(new_relations[key] != old for key, old in old_relations.items())):
        raise AssertionError("0062 changed old relation identity or unexpected new relation")
    ...
    if (None in guard_keys
        ...
        raise AssertionError("0062 must add exactly four AI functions")
    for signature, prior in before[3].items():
        current = after[3][signature]
        if current != prior:
            raise AssertionError("0062 changed old AI function OID/body/ACL: " + signature)
    for key, (signature, definition, definer) in guard_keys.items():
        ...
        if (details[1] != definition.split("$$", 2)[1]
            ...
            raise AssertionError("0062 new function body/security/path drift")
    ...
    for key, signature in TRIGGERS.items():
        ...
        if (target is None or row[3] != target or row[5] != 'O'
            ...
            raise AssertionError("0062 trigger target/enabled/deferral drift")
    with db.cursor() as cursor:
        verify_context(cursor, RuntimeError)
        verify_read(cursor, RuntimeError)
...
with connect() as db:
    ...
    before = snapshot(db)
archive_restore("market_v2_read_before")
with connect("market_v2_read_before") as copy:
    closed(copy, installed=False)
    if restored(snapshot(copy)) != restored(before):
        raise AssertionError("0061 pre-upgrade independent restore differs")
...
archive_restore("market_v2_read_after")
with connect("market_v2_read_after") as copy:
    closed(copy, installed=True)
    restored_after = snapshot(copy)
    if restored(restored_after) != restored(after):
        raise AssertionError("0062 post-upgrade independent restore differs")

MigrationExecutor(connection).migrate(OLD)
with connect() as db:
    closed(db, installed=False)
    if snapshot(db) != before:
        raise AssertionError("0062 empty reverse did not restore exact 0061 state")
MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    closed(db, installed=True)
    ...
    if (replayed[:3] != after[:3]
        ...
        raise AssertionError("0062 reapply differed from first installation")

result = {"upgrade": "0061->0062", "oldAiTables": 82,
    "newAiTables": 83,
    ...
    "oldAiFunctionsOidBodyAclPreservedIncluding0060And0061": True,
    "newFourFunctionsTwoTriggersExact": True,
    "newAttestorNoLoginNoMembership": True,
    "readTableNoReaderWriterAttestorDml": True,
    "oldMaterialAndContextAclClosed": True,
    "newReadRowsAbsentInUpgradeSeed": True,
    "noRealJobExpectedUnder0060Guard": True,
    "readDenialSeparatedToTargetPgTest":
        "ai_assistant.test_business_market_v2_read_receipt_candidate",
    "beforeBackupRestored": True, "afterBackupRestored": True,
    "emptyReverseAndReapply": True, "productionWrites": False}
(folder / "business-market-v2-read-receipt-upgrade-evidence.json").write_text(
