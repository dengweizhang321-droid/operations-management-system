"""Isolated 0057 -> 0058 dormant atomic budget publish upgrade and restore."""
import argparse
import hashlib
import importlib
import json
...
seed = folder / "business-promotion-budget-v10-attestation-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0056->0057"):
    raise RuntimeError("0058 requires verified isolated 0057 predecessor")
if len(OLD_TABLES) != 81 or len(AI_TABLES) != 81:
    raise AssertionError("0058 expected frozen 81-table AI inventory")

OLD = [("ai_assistant", "0057_business_promotion_budget_v10_attestation"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0058_business_promotion_budget_v10_publish_gate"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0057_business_promotion_budget_v10_attestation',"
        "'0058_business_promotion_budget_v10_publish_gate') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0058 requires exact 0057 predecessor without 0058")
...
NEW_FUNCTIONS = (migration.READY_REQUIREMENTS,
...
def archive_restore(name):
    ...
    for executable, arguments in (
        ...
        if done.returncode:
            ...
            raise RuntimeError("isolated 0058 archive/restore failed")


def ready_guards(db, *, installed):
    ...
    expected = ((migration.RUN_GUARD, migration.COMPLETE_GUARD)
                if installed else (closed.RUN_GUARD, closed.COMPLETE_GUARD)) + (
    ...
    for signature, definition in zip(names, expected):
        ...
        should_define = installed and signature == names[1]
        if (row is None or row[0] != definition.split("$$", 2)[1]
                or row[1] is not should_define):
            raise AssertionError("v9/v10 ready guards changed during attestation")
    if installed:
        if ("ai_budget_v10_ready_direct_write_denied" not in migration.RUN_GUARD
                or "session_user<>'teruisi_ai_budget_v10_attestor'" not in migration.RUN_GUARD
                or "ai_budget_v10_ready_requirements" not in migration.COMPLETE_GUARD):
            raise AssertionError("v10 direct writer ready guard changed")
    elif ("ai_promotion_budget_renderer_unpublished" not in closed.RUN_GUARD
            or "ai_promotion_budget_renderer_unpublished" not in closed.COMPLETE_GUARD):
        raise AssertionError("v10 predecessor ready denial changed")
    try:
...
def attestation_closed(db, *, publish_installed):
    ...
    if db.execute("SELECT count(*) FROM pg_catalog.pg_auth_members WHERE "
        ...
        raise AssertionError("0058 verifier role gained membership")
    if db.execute("SELECT count(*) FROM " + attestation.TABLE).fetchone() != (0,):
        raise AssertionError("0058 empty inverse requires no attestation")
    with db.cursor() as cursor:
        (migration.verify_catalog if publish_installed else
            attestation.verify_catalog)(cursor)
    for signature in (migration.READY_SIGNATURE,
                      migration.PUBLISH_SIGNATURE,
                      migration.OUTCOME_SIGNATURE):
        present = db.execute("SELECT to_regprocedure(%s)",
            [signature]).fetchone()[0] is not None
        if present is not publish_installed:
            raise AssertionError("0058 publish function inventory drift")


with connect() as db:
    attestation_closed(db, publish_installed=False)
    ready_guards(db, installed=False)
    before = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
archive_restore("budget_v10_publish_before")
with connect("budget_v10_publish_before") as copy:
    attestation_closed(copy, publish_installed=False)
    ready_guards(copy, installed=False)
    ...
    if (restored[0], restored[1], normalized(restored[2]),
        ...
        raise AssertionError("0057 pre-upgrade independent restore differs")

MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    attestation_closed(db, publish_installed=True)
    ready_guards(db, installed=True)
    after = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
    if after[:2] != before[:2] or after[3] != before[3]:
        raise AssertionError("0058 changed old AI rows, renderer bytes, relations or triggers")
    run_key = db.execute("SELECT to_regprocedure(%s)::text",
        ["public.ai_business_files_guard()"]).fetchone()[0]
    complete_key = db.execute("SELECT to_regprocedure(%s)::text",
        ["public.ai_business_volume_complete_guard()"]).fetchone()[0]
    changed = {run_key, complete_key}
    if None in changed or len(changed) != 2:
        raise AssertionError("0058 file guard identities missing")
    for name, old in before[2].items():
        now = after[2].get(name)
        if now is None:
            raise AssertionError("0058 removed an old AI function: " + name)
        if name == run_key:
            if (old[0] != now[0] or old[2:] != now[2:]
                    or now[1] != migration.RUN_GUARD.split("$$", 2)[1]):
                raise AssertionError("0058 RUN guard OID/ACL/owner or body drift")
        elif name == complete_key:
            if (old[0] != now[0] or old[2:4] != now[2:4]
                    or old[4] is not False or now[4] is not True
                    or old[5:] != now[5:]
                    or now[1] != migration.COMPLETE_GUARD.split("$$", 2)[1]):
                raise AssertionError("0058 complete guard changed beyond body and SECURITY DEFINER")
        elif now != old:
            raise AssertionError("0058 changed unrelated AI function: " + name)
    signatures = (migration.READY_SIGNATURE,
        migration.PUBLISH_SIGNATURE, migration.OUTCOME_SIGNATURE)
    new_keys = {db.execute("SELECT to_regprocedure(%s)::text",
        [signature]).fetchone()[0] for signature in signatures}
    if None in new_keys or len(new_keys) != 3 or (
            set(after[2]) != set(before[2]) | new_keys):
        raise AssertionError("0058 must add exactly three publish functions")
    for signature, definition in zip(signatures, (
            migration.READY_REQUIREMENTS, migration.PUBLISH, migration.OUTCOME)):
        ...
        if (details[1] != definition.split("$$", 2)[1]
            ...
            raise AssertionError("0058 new publish function body or definer drift")
archive_restore("budget_v10_publish_after")
with connect("budget_v10_publish_after") as copy:
    attestation_closed(copy, publish_installed=True)
    ready_guards(copy, installed=True)
    restored = (old_table_digest(copy), old_files(copy), function_catalog(copy),
        relation_trigger_catalog(copy))
    if (restored[0], restored[1], normalized(restored[2]),
            restored_inventory(restored[3])) != (
        ...
        raise AssertionError("0058 post-upgrade independent restore differs")
...
with connect() as db:
    ...
    reverted = (old_table_digest(db), old_files(db), function_catalog(db),
        relation_trigger_catalog(db))
    if reverted != before:
        raise AssertionError("0058 empty reverse did not restore exact 0057 state")
MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    attestation_closed(db, publish_installed=True)
    ready_guards(db, installed=True)
    ...
    if (replayed[0], replayed[1], normalized(replayed[2]),
            ...
            after[3]):
        raise AssertionError("0058 reapply differed from first installation")

result = {"upgrade": "0057->0058", "oldAiTables": 81,
    "oldRowsDigestPreserved": before[0],
    "rendererVersions": list(before[1]), "rendererBytesPreserved": True,
    "unrelatedAiFunctionOidBodyAclPreserved": True,
    "runGuardSameOidAclOwner": True,
    "completeGuardOnlyBodyAndSecurityDefinerChanged": True,
    "v9ReadyDenialPreserved": True, "v10DirectWriterGuardPinned": True,
    "verifierNoLoginNoMembership": True,
    "newPublishFunctionsExactAclOwner": True,
    "beforeBackupRestored": True, "afterBackupRestored": True,
    "emptyReverseAndReapply": True, "productionWrites": False}
(folder / "business-promotion-budget-v10-publish-upgrade-evidence.json").write_text(
