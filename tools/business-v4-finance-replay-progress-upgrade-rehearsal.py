"""Isolated 0047 -> 0048 finance replay extension, restore and empty reverse."""
import argparse
import hashlib
...
seed = folder / "business-v4-replay-progress-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0046->0047"):
    raise RuntimeError("0048 rehearsal requires verified isolated 0047 seed")

OLD = [("ai_assistant", "0047_business_v4_sealer_replay_progress"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0048_business_v4_finance_replay_progress"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0047_business_v4_sealer_replay_progress',"
        "'0048_business_v4_finance_replay_progress') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0048 requires exact 0047 predecessor without 0048")
...
def archive_restore(name):
    ...
    for executable, arguments in (
        ...
        if done.returncode:
            ...
            raise RuntimeError("isolated 0048 archive or restore failed")


def closed(db, *, finance=False):
    with db.cursor() as cursor:
        verify_catalog(cursor, RuntimeError, finance_enabled=finance)
...
archive_restore("v4_finance_replay_before")
with connect("v4_finance_replay_before") as restored:
    closed(restored)
...
with connect() as db:
    closed(db, finance=True)
    assert (old_table_digest(db), old_files(db), old_functions(db)) == before[:3]
    changed = old_functions(db, REPLAY_FUNCTIONS)
    for index, (old, new) in enumerate(zip(before[3], changed)):
        assert new[0] == old[0] and new[2:] == old[2:]
        assert new[1] == (finance_migration.RECORD.split("$$")[1]
                              if index == 2 else old[1])
    assert db.execute("SELECT count(*) FROM ai_business_v4_sealer_replay_progress"
        ).fetchone() == (0,)
archive_restore("v4_finance_replay_after")
with connect("v4_finance_replay_after") as restored:
    closed(restored, finance=True)
    assert (old_table_digest(restored), old_files(restored),
        tuple(row[1:] for row in old_functions(restored)),
        tuple(row[1:] for row in old_functions(restored, REPLAY_FUNCTIONS))) == (
            before[0], before[1], tuple(row[1:] for row in before[2]),
            tuple(row[1:] for row in changed))
...
with connect() as db:
    closed(db, finance=True)
    assert (old_table_digest(db), old_files(db), old_functions(db),
        old_functions(db, REPLAY_FUNCTIONS)) == (*before[:3], changed)

result = {"upgrade": "0047->0048", "aiTables": 79,
    "oldRowsAndRendererBytesPreserved": True,
    "oldFileFunctionOidBodyAndAclPreserved": True,
    "replayFunctionOidAndAclPreserved": True,
    "promotionReplayTestedInUpgrade": False,
...
(folder / "business-v4-finance-replay-progress-upgrade-evidence.json").write_text(
