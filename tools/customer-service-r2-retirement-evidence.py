#!/usr/bin/env python3
"""Prove that the legacy customer-service upload R2 namespace is empty."""
...
FORMAT_VERSION = "customer-service-r2-retirement-evidence-v1"
# Legacy customer-service uploads reused the inventory chunk uploader. Inventory
# itself has retired this whole prefix, so an empty prefix is the only safe
# terminal state and avoids guessing ownership from orphaned object keys.
...
def collect(root: Path) -> dict[str, object]:
    ...
    if object_count or object_bytes or multipart_uploads or multipart_parts:
        raise RuntimeError("legacy customer-service R2 namespace is not empty")
    return {
