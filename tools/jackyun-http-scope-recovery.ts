...
import { auditedHttp890,assert890ReprepareWindow,inspectHttpScopeRecovery,publishHttpScopeRecovery,type HttpScopePermit } from "../lib/jackyun/http-scope-recovery";
...
async function main(){
  ...
  await withJackyunRunLock({runId:"n8n-export-first-890",purpose:"audited_http_scope_recovery",lockDirectory:path.join(root,".runtime/jackyun-automation.lock")},async()=>{
    ...
    const evidence=readN8nPreflightEvidence(path.join(homedir(),".n8n/database.sqlite"),"890");
    ...
    try{
      ...
      if(page.url()==="about:blank")await page.goto("https://web.jackyun.com/home/mainframe_web_horizontal.html",{waitUntil:"domcontentloaded",timeout:30000});
      ...
      const tasks=await readWebSessionTasks(client,"sales",auditedHttp890.exportIntentAt);
      if(!tasks.records.length||tasks.records.some(t=>t.createdAt>=Math.floor(Date.parse(auditedHttp890.exportIntentAt)/1000)*1000))throw Error("原窗口出现任务，拒绝再次准备导出。");
      assert890ReprepareWindow(auditedHttp890.exportIntentAt,tasks.records);
      const observation={observedAt:new Date().toISOString(),tasks:tasks.records.map(({taskId,createdAt})=>({taskId,createdAt}))};
      if(action==="plan"){
        const proposal=await inspectHttpScopeRecovery(root,evidence,observation,new Date().toISOString());
        console.log(JSON.stringify({proposal,approvedSha256:recoverySha(JSON.stringify(proposal))}));
      }else{
        const {proposal}=JSON.parse(await readFile(proposalPath,"utf8")) as {proposal:HttpScopePermit};
        if(proposal.root!==root||Date.now()-Date.parse(proposal.createdAt)>30*60000)throw Error("许可根目录或有效期不符。");
        await publishHttpScopeRecovery(proposal,evidence,sha);
        console.log(JSON.stringify({status:"http_scope_permit_published",runId:"n8n-export-first-890"}));
      }
