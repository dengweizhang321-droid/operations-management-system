...
function Get-WorkerStatusInternal([object]$Identity) {
  ...
  if ($tree.Count -lt 1 -or @($tree | Where-Object {
  ...
  if (-not $workerOwned -or -not $helperOwned) {
    return [pscustomobject]@{
      ...
      HelperPortIds = $helperPortIds; Helper = $null; Receipt = $receipt
    }
  }
  ...
  return [pscustomobject]@{
    ...
    HelperPortIds = $helperPortIds; Helper = $helperContext.Helper; Receipt = $receipt
  }
}
  ...
  if ($Action -eq "Start") {
    ...
    try {
      ...
      for ($attempt = 0; $attempt -lt 360; $attempt++) {
        ...
        if ($readyStatus.State -eq "exact_release") { break }
        if ($readyStatus.State -ne "starting_exact_release") { break }
      }
      if (-not $readyStatus -or $readyStatus.State -ne "exact_release") {
        throw "Immutable Worker/helper failed to reach exact 3000/5791 ownership within 90 seconds"
      }
