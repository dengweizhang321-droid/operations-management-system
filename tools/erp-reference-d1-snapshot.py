"""Create a consistent, read-only D1 erp-reference migration snapshot."""
...
FORMAT_VERSION = "erp-reference-d1-snapshot-v1"
...
def snapshot(source_path: Path, output_path: Path, manifest_path: Path) -> dict[str, object]:
    ...
    try:
        ...
        if authority_row is None or dict(authority_row) != {"id": 1, "owner": "legacy", "epoch": 1, "cutover_id": ""}:
            raise RuntimeError("D1 erp-reference authority is not the frozen legacy state")
        counts = {
            "batches": int(source.execute("SELECT COUNT(*) FROM erp_reference_import_batches").fetchone()[0]),
            "products": int(source.execute("SELECT COUNT(*) FROM erp_product_master").fetchone()[0]),
            "combos": int(source.execute("SELECT COUNT(*) FROM erp_combo_items").fetchone()[0]),
            "projectionOutbox": int(source.execute("SELECT COUNT(*) FROM erp_reference_projection_outbox").fetchone()[0]),
            "fingerprints": int(source.execute("SELECT COUNT(*) FROM import_content_fingerprints WHERE domain='erp-reference'").fetchone()[0]),
            "attempts": int(source.execute("SELECT COUNT(*) FROM import_content_attempts WHERE domain='erp-reference'").fetchone()[0]),
            "heads": int(source.execute("SELECT COUNT(*) FROM import_scope_heads WHERE domain='erp-reference'").fetchone()[0]),
        }
        ...
        try:
            ...
            if destination.execute("PRAGMA integrity_check").fetchone()[0].lower() != "ok":
                raise RuntimeError("erp-reference snapshot integrity check failed")
        finally:
            destination.close()
    finally:
        source.close()
