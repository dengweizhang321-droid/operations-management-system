"""Isolated 0046 -> 0047 closed replay ledger, restore and empty reverse."""
import argparse
import hashlib
import json
import os
...
from ai_assistant.table_manifest import AI_TABLES_PRE_V4_REPLAY_PROGRESS
...
seed = folder / "business-promotion-trial-file-upgrade-evidence.json"
if (ROOT.resolve() == Path(r"D:\运营管理系统").resolve()
        ...
        or not seed.is_file()
        or json.loads(seed.read_text(encoding="utf-8")).get("upgrade") != "0045->0046"):
    raise RuntimeError("0047 rehearsal requires verified isolated 0046 seed")

OLD = [("ai_assistant", "0046_business_promotion_trial_file_guard"),
       ("finance", "0004_finance_revision_monotonic"),
       ("netshop", "0003_netshop_source_revision_guard")]
NEW = [("ai_assistant", "0047_business_v4_sealer_replay_progress"), *OLD[1:]]
with connection.cursor() as cursor:
    cursor.execute("SELECT name FROM django_migrations WHERE app='ai_assistant' "
        "AND name IN ('0046_business_promotion_trial_file_guard',"
        "'0047_business_v4_sealer_replay_progress') ORDER BY name")
    if [row[0] for row in cursor.fetchall()] != [OLD[0][1]]:
        raise RuntimeError("0047 requires exact 0046 predecessor without 0047")
...
def old_table_digest(db):
    material = {}
    for table in sorted(AI_TABLES_PRE_V4_REPLAY_PROGRESS):
...
def old_functions(db):
    values = []
    for signature in PREVIOUS_FUNCTIONS:
...
def archive_restore(name):
    ...
    for executable, arguments in (
        ...
        if done.returncode:
            ...
            raise RuntimeError("isolated 0047 archive or restore failed")
...
with connect() as db:
    assert len(AI_TABLES_PRE_V4_REPLAY_PROGRESS) == 78
    before = (old_table_digest(db), old_files(db), old_functions(db))
    assert db.execute("SELECT to_regclass('public.ai_business_v4_sealer_replay_progress')"
        ).fetchone() == (None,)
archive_restore("v4_replay_progress_before")
with connect("v4_replay_progress_before") as restored:
...
with connect() as db:
    closed(db)
    assert (old_table_digest(db), old_files(db), old_functions(db)) == before
    assert db.execute("SELECT count(*) FROM ai_business_v4_sealer_replay_progress"
        ).fetchone() == (0,)
archive_restore("v4_replay_progress_after")
with connect("v4_replay_progress_after") as restored:
    closed(restored)
    assert (old_table_digest(restored), old_files(restored),
        tuple(row[1:] for row in old_functions(restored))) == (
            before[0], before[1], tuple(row[1:] for row in before[2]))
...
with connect() as db:
    ...
    assert all(db.execute("SELECT to_regprocedure(%s)", [signature]).fetchone()
        == (None,) for signature in NEW_FUNCTIONS)
MigrationExecutor(connection).migrate(NEW)
with connect() as db:
    closed(db)
    assert (old_table_digest(db), old_files(db), old_functions(db)) == before

result = {"upgrade": "0046->0047", "oldAiTables": 78,
    "newAiTables": 79, "oldRowsAndRendererBytesPreserved": True,
    "oldFunctionOidBodyAndAclPreserved": True,
...
(folder / "business-v4-replay-progress-upgrade-evidence.json").write_text(
