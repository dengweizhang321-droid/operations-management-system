...
let principal = principalA, dropEvidence = false, dropReport = false, detailDelay = "", flipAfterPreflight = false, rejectEvidence = 0, previewDelay = 0;
...
function makePreview(body) {
  ...
  return {principalKey:principal,canCollect:sources.length>0&&sources.length<=12,planDigest:"d".repeat(64),request:body,coverage,
    capacity:{sourceCount:sources.length,maxSources:12,planBytes:4000,maxPlanBytes:16000,workflowBytes:5000,maxWorkflowBytes:8000},
    limitations:["尚未采集，不代表业务记录已存在。",...(sources.length>12?["来源数量超过上限12；完整请求保留，不自动缩小。"]:[])],
    evidenceRequest:{sources,collectionMode:"bulk",autoCollect:true,analysisRequest:{schemaVersion:"business-analysis-request-v1",question:body.question,requestedDimensions:["shop","category","spu","sku","keyword"],requestedWindows:body.windows}}};
}
const server=http.createServer(async(req,res)=>{
  try {
    ...
    if(url.pathname.startsWith("/api/")){
      ...
      if(url.pathname==="/api/ai/business-plan/preview"){const result=makePreview(body),delay=previewDelay;previewDelay=0;if(delay)await new Promise(resolve=>setTimeout(resolve,delay));return send(result);}
      ...
      if(url.pathname==="/api/ai/business-evidence"&&req.method==="POST"){
        ...
        if(!row){row=item("evidence-"+(evidence.length+1),body.analysisRequest.question);row.clientRequestId=body.clientRequestId;row.plan={...body,collector:{version:1,surface:"business_collection",pageSize:100}};row.owner=principal;evidence.unshift(row);}
      ...
      const found=url.pathname.match(/^\/api\/ai\/business-evidence\/([^/]+)(?:\/(control|finish))?$/);
      if(found){
...
const check=async(name,task)=>{await task();console.log(JSON.stringify({case:name,passed:true}));};
try{
 ...
 await check("explicit_source_preview",async()=>{
  await page.getByRole("button",{name:"预览完整来源计划",exact:true}).click();await page.getByText("拟采集 2 / 12 个来源",{exact:false}).waitFor();
 ...
 await check("exact_erp_market_scope_editing",async()=>{
  ...
  await page.getByRole("button",{name:"预览完整来源计划",exact:true}).click();await page.getByText("拟采集 4 / 12 个来源",{exact:false}).waitFor();const matrix=await page.getByRole("region",{name:"来源范围预览"}).innerText();assert.match(matrix,/店铺：sku/);assert.match(matrix,/ERP渠道：current/);assert.match(matrix,/类目：合成三级类目/);
 ...
 await check("over_capacity_keeps_all_sources",async()=>{
  ...
  await page.getByLabel("同比",{exact:true}).check();await page.getByLabel("环比",{exact:true}).check();await page.getByRole("button",{name:"预览完整来源计划",exact:true}).click();await page.getByText("拟采集 16 / 12 个来源",{exact:false}).waitFor();
  assert.equal(await page.getByRole("region",{name:"来源范围预览"}).getByRole("cell",{name:"尚未取数",exact:true}).count(),16);assert.equal(await page.getByRole("button",{name:"确认范围并开始后台采集",exact:true}).isDisabled(),true);
 });
 ...
 await check("principal_storage_isolation",async()=>{
  await page.reload();await page.getByLabel("分析问题",{exact:true}).fill("账号A待确认问题");await page.getByLabel("开始日期",{exact:true}).fill("2026-09-01");await page.getByLabel("结束日期",{exact:true}).fill("2026-09-15");await page.getByLabel("店铺 1 精确名称",{exact:true}).fill("账号A合成店铺");await page.getByRole("button",{name:"预览完整来源计划",exact:true}).click();await page.getByText("拟采集 2 / 12 个来源",{exact:false}).waitFor();await page.getByLabel("我已核对精确范围与限制，确认采集以上全部来源").check();dropEvidence=true;await page.getByRole("button",{name:"确认范围并开始后台采集",exact:true}).click();await page.getByTestId("write-error").waitFor();
 ...
 await check("principal_changes_between_preflight_and_post",async()=>{
  await page.getByLabel("分析问题",{exact:true}).fill("账号切换边界");await page.getByLabel("开始日期",{exact:true}).fill("2026-09-01");await page.getByLabel("结束日期",{exact:true}).fill("2026-09-15");await page.getByLabel("店铺 1 精确名称",{exact:true}).fill("合成账号B店铺");await page.getByRole("button",{name:"预览完整来源计划",exact:true}).click();await page.getByText("拟采集 2 / 12 个来源",{exact:false}).waitFor();
 ...
 assert.deepEqual(pageErrors,[]);await fs.writeFile(path.join(directory,"evidence.json"),JSON.stringify({passed:true,cases:13,apiRequests:requests.length,evidenceCreates:count("/api/ai/business-evidence"),reportCreates:count("/api/ai/business-reports"),pageErrors,syntheticOnly:true},null,2));
