"""Verify 0010 -> 0011, real roles and restore in a fresh synthetic database."""
import argparse
import hashlib
import json
import os
...
old_target = [("ai_assistant", "0010_dingtalk_schedules")]
...
target = [("ai_assistant", "0011_prompt_settings")]
executor = MigrationExecutor(connection)
...
assert current.get_model("ai_assistant", "AiPromptSettingsRevision").objects.count() == 0
...
insert = "INSERT INTO ai_prompt_settings_revisions(version,config_json,created_by,created_at) VALUES (1,%s,'fixture',now())"
for role, password in (("reader", reader_password), ("writer", writer_password)):
    ...
    with psycopg.connect(url, autocommit=True) as limited:
        ...
        if role == "writer":
            ...
            assert limited.execute("SELECT count(*) FROM ai_prompt_settings_revisions").fetchone()[0] == 1
        else:
            denied(limited, insert, [json.dumps(DEFAULT_CONFIG)])
            limited.execute("SELECT version,config_json FROM ai_prompt_settings_revisions")
        for query in ("UPDATE ai_prompt_settings_revisions SET created_by='other'", "DELETE FROM ai_prompt_settings_revisions", "TRUNCATE ai_prompt_settings_revisions", "UPDATE sales_order_lines SET allocated_amount_cents=0", "CREATE TABLE public.forbidden_test(id int)"):
...
dump = run_root / "prompt-settings.dump"
for executable, arguments in [("pg_dump.exe", ["-Fc", "-f", str(dump), "teruisi_ai_rehearsal"]), ("createdb.exe", ["teruisi_prompt_restore"]), ("pg_restore.exe", ["--exit-on-error", "-d", "teruisi_prompt_restore", str(dump)])]:
    subprocess.run([str(bin_path / executable), *arguments], check=True, capture_output=True, timeout=60)
assert snapshot("teruisi_prompt_restore") == before_digest
print(json.dumps({"upgrade":"0010->0011", "existingConversationsAndMessagesPreserved":True, "defaultsRequireNoSeed":True,
