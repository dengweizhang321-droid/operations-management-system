...
const root=process.cwd(), directory=path.resolve(process.argv[2]||".runtime/business-sales-picker-ui");
...
import Picker from ${JSON.stringify(path.join(root,"app/ai-business-sales-picker.tsx"))};
...
function fixture(query,cursor){
  const revision=mode==="revision"?"8:9":"7:9";
  const generation=mode==="generation"?"d".repeat(32):"c".repeat(32),directoryDigest=mode==="directory"?"f".repeat(64):"e".repeat(64);
  let items=Array.from({length:count},(_,i)=>{const identity={platform:"京东",shop:`合成店铺${String(i).padStart(4,"0")}`,channel:"B端直营"};return {optionKey:sha({domain:"sales",identity}),identity,source:"erp_sales",sourceDataset:"sales_order_lines",
    dateMetadata:{kind:"current_fact_business_date_envelope",firstDate:"2026-08-01",lastDate:"2026-08-03",snapshotDate:null,coverageVerified:false},
  ...
  const page={schemaVersion:"business-analysis-options-v1",domain:"sales",authorityVerified:true,revision,directoryGeneration:generation,directoryDigest,query,queryDigest:sha(query),items,
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname.startsWith("/api/")){
      requests.push({method:req.method,path:url.pathname,query:url.search});assert.equal(req.method,"GET");assert.equal(url.pathname,"/api/ai/business-plan/sales-options");
      ...
      if(mode==="not-ready")return send({code:"options_not_ready",error:"ERP目录尚未初始化"},503);
      const value={schemaVersion:"business-sales-options-response-v1",principalKey:mode==="wrong-key"?keyB:actor,page};
...
const picker=()=>page.getByRole("region",{name:"选择ERP分析来源",exact:true});
const add=()=>picker().getByRole("button",{name:/^添加ERP来源 /});
const events=async()=>JSON.parse(await page.locator("#events").innerText());
const ready=async(expected=20)=>{await page.waitForFunction(n=>[...document.querySelectorAll('button[aria-label^="添加ERP来源 "]')].length===n,expected);await page.getByRole("button",{name:"取消ERP来源读取",exact:true}).waitFor({state:"detached"});};
...
try{
  ...
  await check("manual_selection_revalidates_identity_and_never_changes_dates_or_submits",async()=>{
    ...
    assert.equal(requests.length,before+1);assert.equal((await events())[0].identity.shop,"合成店铺0000");
    ...
    assert.equal((await events())[0].identity.channel,"B端直营");assert.equal((await events())[0].directoryGeneration,"c".repeat(32));
  ...
  await check("twenty_then_three_with_previous_and_exact_filters",async()=>{
    await picker().getByRole("button",{name:"下一页ERP来源",exact:true}).click();await ready(3);assert.match(await add().first().getAttribute("aria-label"),/0020/);
    await picker().getByRole("button",{name:"上一页ERP来源",exact:true}).click();await ready();assert.match(await add().first().getAttribute("aria-label"),/0000/);
    await picker().getByLabel("ERP精确平台",{exact:true}).fill("京东");await picker().getByLabel("ERP精确店铺",{exact:true}).fill("合成店铺0022");await picker().getByLabel("ERP精确渠道",{exact:true}).fill("B端直营");await read();await ready(1);
    assert.equal(new URLSearchParams(requests.at(-1).query).get("channel"),"B端直营");assert.match(await add().first().getAttribute("aria-label"),/0022/);
    await picker().getByLabel("ERP精确店铺",{exact:true}).fill("无此名称");await read();await ready(0);await picker().getByText("当前筛选没有已发布来源；这不等于没有经营活动。",{exact:true}).waitFor();
  });
  ...
  await check("revision_409_requires_explicit_first_page_reread",async()=>{
    mode="conflict";await picker().getByRole("button",{name:"下一页ERP来源",exact:true}).click();await picker().getByRole("alert").filter({hasText:"从首页重读"}).waitFor();assert.equal(await add().count(),0);
  ...
  await check("late_old_filter_response_cannot_replace_new_page",async()=>{
    await reload();delay=true;started=false;await read();await waitDelay();await picker().getByLabel("ERP精确平台",{exact:true}).fill("京东");await picker().getByLabel("ERP精确店铺",{exact:true}).fill("合成店铺0022");await read();await ready(1);await waitFinished();assert.equal(await add().count(),1);assert.match(await add().first().getAttribute("aria-label"),/0022/);
  });
  await check("cancel_pending_selection_never_emits",async()=>{
    await reload();delay=true;started=false;await add().first().click();await waitDelay();await picker().getByRole("button",{name:"取消ERP来源读取",exact:true}).click();await waitFinished();assert.equal((await events()).length,0);assert.equal(await add().count(),0);
  });
  ...
  await check("hundred_page_bound_reports_remaining_instead_of_clipping",async()=>{
    count=2001;await read();await ready();for(let n=2;n<=100;n++){await picker().getByRole("button",{name:"下一页ERP来源",exact:true}).click();await picker().getByText(`第 ${n} 页 · 本页 20 个来源 · 后面还有来源`,{exact:true}).waitFor();}
    assert.equal(await picker().getByRole("button",{name:"下一页ERP来源",exact:true}).isDisabled(),true);await picker().getByText("已浏览 100 页，仍有来源未展示。请缩小筛选后从首页重读。",{exact:true}).waitFor();assert.equal(await add().count(),20);
  });
